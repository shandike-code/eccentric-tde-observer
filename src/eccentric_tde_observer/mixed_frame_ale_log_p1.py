"""对数频率守恒 P1 变量的完整 Lorentz 源项与一维 ALE 推进。"""
...
class MixedFrameALELogP1Step:
    """一个 ``Q=nu*I_nu`` 对数频率 P1 的后向 Euler ALE 辐射步。"""

    final_lab_mean_intensity_energy_density: NDArray[np.float64]
    final_lab_first_moment_intensity_energy_density: NDArray[np.float64]
    final_lab_mean_intensity_energy_angle_average: NDArray[np.float64]
    final_lab_first_moment_intensity_energy_angle_average: NDArray[np.float64]
    final_comoving_angle_mean_intensity_energy_density: NDArray[np.float64]
    final_comoving_angle_first_moment_intensity_energy_density: NDArray[np.float64]
    final_comoving_mean_intensity_energy_density: NDArray[np.float64]
    final_comoving_mean_first_moment_intensity_energy_density: NDArray[np.float64]
    lab_extinction_mean_per_cm: NDArray[np.float64]
    lab_extinction_first_moment_per_cm: NDArray[np.float64]
    lab_emissivity_energy_mean_cgs: NDArray[np.float64]
    lab_emissivity_energy_first_moment_cgs: NDArray[np.float64]
    left_ale_face_mean_intensity_energy: NDArray[np.float64]
    left_ale_face_first_moment_intensity_energy: NDArray[np.float64]
    right_ale_face_mean_intensity_energy: NDArray[np.float64]
    right_ale_face_first_moment_intensity_energy: NDArray[np.float64]
    initial_radiation_energy_group_erg_cm2: NDArray[np.float64]
    final_radiation_energy_group_erg_cm2: NDArray[np.float64]
    left_ale_energy_flux_group_cgs: NDArray[np.float64]
    right_ale_energy_flux_group_cgs: NDArray[np.float64]
    material_heating_group_erg_s_cm2: NDArray[np.float64]
    energy_ledger_residual_group_erg_cm2: NDArray[np.float64]
    ...
    minimum_reconstructed_intensity_energy_density: float
...
def _broadcast_realizable_log_p1(
    name: str,
    ...
    try:
        ...
        raise PhysicalDomainError(
            f"{name}_first_moment cannot broadcast to {shape}"
        ) from error
    if not np.all(np.isfinite(moment)):
        raise PhysicalDomainError(f"{name}_first_moment must be finite")
    state = limit_nonnegative_log_frequency_group_p1(mean, moment)
...
def _node_view(values: NDArray[np.float64]) -> NDArray[np.float64]:
    """把 ``(group,2,angle,depth)`` 合并成空间扫掠器的谱轴。"""
...
def solve_mixed_frame_ale_log_p1_group_step(
    ...
    initial_active_lab_mean_intensity_energy_density: ArrayLike,
    initial_active_lab_first_moment_intensity_energy_density: ArrayLike,
    outer_lab_guard_mean_intensity_energy_density: ArrayLike,
    outer_lab_guard_first_moment_intensity_energy_density: ArrayLike,
    comoving_true_absorption_mean_per_cm: ArrayLike,
    comoving_true_absorption_first_moment_per_cm: ArrayLike,
    comoving_thermal_emissivity_energy_mean_cgs: ArrayLike,
    comoving_thermal_emissivity_energy_first_moment_cgs: ArrayLike,
    ...
    duration_s: float,
    *,
    left_exterior_mean_intensity_energy: ArrayLike = 0.0,
    left_exterior_first_moment_intensity_energy: ArrayLike = 0.0,
    right_exterior_mean_intensity_energy: ArrayLike = 0.0,
    right_exterior_first_moment_intensity_energy: ArrayLike = 0.0,
    ...
    iteration_observer: Callable[
        [int, NDArray[np.float64], NDArray[np.float64]], None
    ]
    | None = None,
    source_iteration_initial_mean_guess: ArrayLike | None = None,
    source_iteration_initial_first_moment_guess: ArrayLike | None = None,
    diagnostic_fixed_iteration_count: int | None = None,
) -> MixedFrameALELogP1Step:
    """联立推进对数频率能量密度、共动碰撞源与 ALE 移动通量。

    三个 ``apply_*_lorentz`` 参数只供分量级诊断；正式物理路径必须保持默认值。
    ``source_iteration_initial_*_guess`` 与 ``diagnostic_fixed_iteration_count``
    ...
    initial_mean, initial_moment = _broadcast_realizable_log_p1(
        "initial_active_lab_intensity_energy",
        initial_active_lab_mean_intensity_energy_density,
        initial_active_lab_first_moment_intensity_energy_density,
        active_shape,
    )
    outer_template_mean, outer_template_moment = _broadcast_realizable_log_p1(
        "outer_lab_guard_intensity_energy",
        outer_lab_guard_mean_intensity_energy_density,
        outer_lab_guard_first_moment_intensity_energy_density,
        outer_shape,
    )
    absorption_mean, absorption_moment = _broadcast_realizable_log_p1(
        "comoving_true_absorption",
        comoving_true_absorption_mean_per_cm,
        comoving_true_absorption_first_moment_per_cm,
        collision_shape,
    )
    thermal_mean, thermal_moment = _broadcast_realizable_log_p1(
        "comoving_thermal_emissivity_energy",
        comoving_thermal_emissivity_energy_mean_cgs,
        comoving_thermal_emissivity_energy_first_moment_cgs,
        collision_shape,
    )
    scattering_mean, scattering_moment = _broadcast_realizable_log_p1(
        "comoving_scattering",
    ...
    left_mean, left_moment = _broadcast_realizable_log_p1(
        "left_exterior_intensity_energy",
        left_exterior_mean_intensity_energy,
        left_exterior_first_moment_intensity_energy,
        (active_groups, angle_points),
    )
    right_mean, right_moment = _broadcast_realizable_log_p1(
        "right_exterior_intensity_energy",
        right_exterior_mean_intensity_energy,
        right_exterior_first_moment_intensity_energy,
        (active_groups, angle_points),
    )
    if periodic_spatial_boundary and any(
        np.any(values != 0.0)
        for values in (left_mean, left_moment, right_mean, right_moment)
    ):
        raise PhysicalDomainError(
    ...
    # 中文：逐项关闭 Lorentz 变换只用于定位误差，不能作为生产物理闭合。
    ...
    extinction_state = limit_nonnegative_log_frequency_group_p1(
        extinction_mean_comoving, extinction_moment_comoving
    )
    if extinction_state.limited_group_count != 0:
        raise ArithmeticError("sum of realizable extinctions became non-realizable")
    collision_angle_shape = (collision_groups, angle_points, depth_points)
    lab_extinction = lorentz_translate_comoving_log_p1_extinction_to_lab(
        np.broadcast_to(
            extinction_mean_comoving[:, None, :], collision_angle_shape
        ),
        np.broadcast_to(
            extinction_moment_comoving[:, None, :], collision_angle_shape
        ),
    ...
    initial_nodes = log_frequency_group_p1_gauss_node_values(
    ...
    lab_extinction_nodes = log_frequency_group_p1_gauss_node_values(
    ...
    scattering_nodes = log_frequency_group_p1_gauss_node_values(
        scattering_mean, scattering_moment
    )

    if (
        source_iteration_initial_mean_guess is None
    ) != (
        source_iteration_initial_first_moment_guess is None
    ):
        raise PhysicalDomainError(
            "both source-iteration log-P1 guess moments must be supplied together"
        )
    if source_iteration_initial_mean_guess is None:
        current_mean = np.array(initial_mean, copy=True)
        current_moment = np.array(initial_moment, copy=True)
    else:
        current_mean, current_moment = _broadcast_realizable_log_p1(
            "source_iteration_initial_intensity_energy",
            source_iteration_initial_mean_guess,
            source_iteration_initial_first_moment_guess,
            active_shape,
        )
    if iteration_observer is not None:
        iteration_observer(
            0,
            _readonly(current_mean.view()),
            _readonly(current_moment.view()),
        )
    final_change = np.inf
    ...
    for iteration in range(1, iteration_limit + 1):
        ...
        comoving = comoving_log_frequency_group_p1_radiation(
            outer_mean,
            outer_moment,
        ...
        comoving_mean_nodes = log_frequency_group_p1_gauss_node_values(
            comoving.mean_intensity_density,
            comoving.mean_intensity_first_moment_density,
        )
        # 中文：两个 P1 的乘积用两点 Gauss 正权积分后投影回 P1。
        emissivity_nodes = thermal_nodes + scattering_nodes * comoving_mean_nodes
        emissivity = log_frequency_group_p1_from_gauss_node_values(
            emissivity_nodes
        )
        iteration_limiter, maximum_ratio = _record_limiter(
            iteration_limiter, maximum_ratio, emissivity
        )
        lab_emissivity = lorentz_translate_comoving_log_p1_emissivity_to_lab(
            np.broadcast_to(emissivity.mean_density[:, None, :], collision_angle_shape),
            np.broadcast_to(
                emissivity.first_moment_density[:, None, :], collision_angle_shape
            ),
            stencil.comoving_collision_edge_hz,
            stencil.active_lab_edge_hz,
            emissivity_doppler,
        ...
        lab_emissivity_nodes = log_frequency_group_p1_gauss_node_values(
        ...
        updated = log_frequency_group_p1_from_gauss_node_values(updated_nodes)
        iteration_limiter, maximum_ratio = _record_limiter(
        ...
        change = max(
            ...
            float(np.max(np.abs(updated.first_moment_density - current_moment))),
        )
        ...
        if diagnostic_fixed_iteration_count is None:
            raise ArithmeticError(
                "mixed-frame log-P1 ALE source iteration did not converge: "
                f"iterations={maximum_iterations}"
            )
        fixed_point_converged = final_change <= tolerance

    # 中文：最终源项与残差使用同一强度状态，避免错位能量账本。
    ...
    emissivity_nodes = thermal_nodes + scattering_nodes * comoving_mean_nodes
    emissivity = log_frequency_group_p1_from_gauss_node_values(emissivity_nodes)
    final_limiter, maximum_ratio = _record_limiter(
    ...
    final_nodes = log_frequency_group_p1_gauss_node_values(
        current_mean, current_moment
    )
    lab_emissivity_nodes = log_frequency_group_p1_gauss_node_values(
        lab_emissivity.mean_density, lab_emissivity.first_moment_density
    )
    ...
    left_face = log_frequency_group_p1_from_gauss_node_values(left_face_nodes)
    right_face = log_frequency_group_p1_from_gauss_node_values(right_face_nodes)
    final_limiter, maximum_ratio = _record_limiter(
    ...
    active_log_width = np.diff(np.log(stencil.active_lab_edge_hz))
    total_ledger = float(np.sum(active_log_width * ledger))
    total_ledger_scale = max(
        float(np.sum(active_log_width * np.abs(initial_energy))),
        float(np.sum(active_log_width * np.abs(final_energy))),
        float(duration * np.sum(active_log_width * np.abs(right_flux - left_flux))),
        float(duration * np.sum(active_log_width * np.abs(material_heating))),
    )
    ...
    comoving_angle_nodes = log_frequency_group_p1_gauss_node_values(
        comoving.angle_mean_density, comoving.angle_first_moment_density
    )
    extinction_comoving_nodes = log_frequency_group_p1_gauss_node_values(
        extinction_mean_comoving, extinction_moment_comoving
    )
    comoving_collision_nodes = (
        emissivity_nodes[:, :, None, :]
        - extinction_comoving_nodes[:, :, None, :] * comoving_angle_nodes
    )
    lab_source_energy = np.pi * np.einsum(
        "f,m,fgmd->d", active_log_width, weight, lab_collision_nodes
    )
    ...
    collision_log_width = np.diff(np.log(stencil.comoving_collision_edge_hz))
    ...
    minimum_reconstructed = float(
        np.min(current_mean - 3.0 * np.abs(current_moment))
    )
    ...
    if not all(np.all(np.isfinite(array)) for array in arrays):
        raise ArithmeticError("mixed-frame log-P1 ALE diagnostics became non-finite")
    return MixedFrameALELogP1Step(
        final_lab_mean_intensity_energy_density=_readonly(current_mean),
        final_lab_first_moment_intensity_energy_density=_readonly(current_moment),
        final_lab_mean_intensity_energy_angle_average=_readonly(lab_angle_mean),
        final_lab_first_moment_intensity_energy_angle_average=_readonly(
            lab_angle_moment
        ),
        final_comoving_angle_mean_intensity_energy_density=(
            comoving.angle_mean_density
        ),
        final_comoving_angle_first_moment_intensity_energy_density=(
            comoving.angle_first_moment_density
        ),
        final_comoving_mean_intensity_energy_density=(
            comoving.mean_intensity_density
        ),
        final_comoving_mean_first_moment_intensity_energy_density=(
            comoving.mean_intensity_first_moment_density
        ),
        ...
        lab_emissivity_energy_mean_cgs=lab_emissivity.mean_density,
        lab_emissivity_energy_first_moment_cgs=(
            lab_emissivity.first_moment_density
        ),
        left_ale_face_mean_intensity_energy=left_face.mean_density,
        left_ale_face_first_moment_intensity_energy=left_face.first_moment_density,
        right_ale_face_mean_intensity_energy=right_face.mean_density,
        right_ale_face_first_moment_intensity_energy=(
            right_face.first_moment_density
        ),
        initial_radiation_energy_group_erg_cm2=_readonly(initial_energy),
        final_radiation_energy_group_erg_cm2=_readonly(final_energy),
        left_ale_energy_flux_group_cgs=_readonly(left_flux),
        right_ale_energy_flux_group_cgs=_readonly(right_flux),
        material_heating_group_erg_s_cm2=_readonly(material_heating),
        energy_ledger_residual_group_erg_cm2=_readonly(ledger),
        ...
        minimum_reconstructed_intensity_energy_density=minimum_reconstructed,
