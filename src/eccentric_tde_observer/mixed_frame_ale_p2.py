"""组内二次频谱的完整 Lorentz 源项与一维 ALE 守恒推进。

每个频率控制体保存组平均和两个归一化 Legendre 矩。三点 Gauss 节点
推进 P2 强度，四点 Gauss 规则则用于精确投影两个 P2 函数的乘积。
"""
...
_P2_NODE_WEIGHT = np.array([5.0 / 18.0, 4.0 / 9.0, 5.0 / 18.0])
...
class MixedFrameALEP2Step:
    """一个组内 P2 频谱的后向 Euler 混合系 ALE 辐射步。"""

    final_lab_mean_intensity_density: NDArray[np.float64]
    final_lab_first_moment_intensity_density: NDArray[np.float64]
    final_lab_second_moment_intensity_density: NDArray[np.float64]
    ...
    final_lab_second_moment_intensity_angle_average: NDArray[np.float64]
    ...
    final_comoving_angle_second_moment_intensity_density: NDArray[np.float64]
...
def _broadcast_realizable_p2(
    ...
    moments: list[NDArray[np.float64]] = []
    for order, values in (("first", first_values), ("second", second_values)):
        try:
            moment = np.array(
                np.broadcast_to(np.asarray(values, dtype=np.float64), shape),
                copy=True,
            )
        except ValueError as error:
            raise PhysicalDomainError(
                f"{name}_{order}_moment cannot broadcast to {shape}"
            ) from error
        if not np.all(np.isfinite(moment)):
            raise PhysicalDomainError(f"{name}_{order}_moment must be finite")
        moments.append(moment)
    state = limit_nonnegative_frequency_group_p2(mean, moments[0], moments[1])
    if state.limited_group_count != 0:
        raise PhysicalDomainError(f"{name} must be non-negative realizable")
    return mean, moments[0], moments[1]


def _node_view(values: NDArray[np.float64]) -> NDArray[np.float64]:
    """把 ``(group,3,angle,depth)`` 合并成空间扫掠器的频率轴。"""
    return values.reshape(values.shape[0] * 3, *values.shape[2:])


def _record_limiter(count: int, ratio: float, state) -> tuple[int, float]:
    return (
        count + int(state.limited_group_count),
        max(ratio, float(state.maximum_prelimit_negativity_ratio)),
    )


def _project_p2_product_nodes(
    values: NDArray[np.float64],
    normalized_node: NDArray[np.float64],
    normalized_weight: NDArray[np.float64],
):
    """用四点 Gauss 规则把最高六次的 P2 乘积投影回 P2。"""
    view = (1, normalized_node.size) + (1,) * (values.ndim - 2)
    x = normalized_node.reshape(view)
    weight = normalized_weight.reshape(view)
    mean = np.sum(weight * values, axis=1)
    first = np.sum(weight * values * x, axis=1)
    second = np.sum(weight * values * 0.5 * (3.0 * x**2 - 1.0), axis=1)
    return limit_nonnegative_frequency_group_p2(mean, first, second)


def solve_mixed_frame_ale_p2_group_step(
    ...
    *,
    ...
    left_exterior_second_moment_intensity: ArrayLike = 0.0,
    right_exterior_mean_intensity: ArrayLike = 0.0,
    right_exterior_first_moment_intensity: ArrayLike = 0.0,
    right_exterior_second_moment_intensity: ArrayLike = 0.0,
    ...
    """联立求解 P2 频谱的 Lorentz 碰撞源、ALE 储能和空间通量。"""
    ...
    angle_weight = np.asarray(angular_weight, dtype=np.float64)
    if (
        mu.ndim != 1
        or angle_weight.shape != mu.shape
        or mu.size < 2
        or not np.all(np.isfinite(mu))
        or not np.all(np.isfinite(angle_weight))
        or np.any(mu <= -1.0)
        or np.any(mu >= 1.0)
        or np.any(angle_weight <= 0.0)
        or not np.isclose(np.sum(angle_weight), 2.0, rtol=2.0e-13, atol=2.0e-15)
    ):
    ...
    initial_mean, initial_first, initial_second = _broadcast_realizable_p2(
        "initial_active_lab_intensity",
        initial_active_lab_mean_intensity_density,
        initial_active_lab_first_moment_intensity_density,
        initial_active_lab_second_moment_intensity_density,
        active_shape,
    )
    outer_mean_template, outer_first_template, outer_second_template = (
        _broadcast_realizable_p2(
            "outer_lab_guard_intensity",
            outer_lab_guard_mean_intensity_density,
            outer_lab_guard_first_moment_intensity_density,
            outer_lab_guard_second_moment_intensity_density,
            outer_shape,
        )
    )
    absorption_mean, absorption_first, absorption_second = _broadcast_realizable_p2(
        "comoving_true_absorption",
        comoving_true_absorption_mean_per_cm,
        comoving_true_absorption_first_moment_per_cm,
        comoving_true_absorption_second_moment_per_cm,
        collision_shape,
    )
    thermal_mean, thermal_first, thermal_second = _broadcast_realizable_p2(
        "comoving_thermal_emissivity",
        comoving_thermal_emissivity_mean_cgs,
        comoving_thermal_emissivity_first_moment_cgs,
        comoving_thermal_emissivity_second_moment_cgs,
        collision_shape,
    )
    scattering_mean, scattering_first, scattering_second = _broadcast_realizable_p2(
        "comoving_scattering",
        comoving_scattering_mean_per_cm,
        comoving_scattering_first_moment_per_cm,
        comoving_scattering_second_moment_per_cm,
        collision_shape,
    )
    ...
    left_mean, left_first, left_second = _broadcast_realizable_p2(
        "left_exterior_intensity",
        left_exterior_mean_intensity,
        left_exterior_first_moment_intensity,
        left_exterior_second_moment_intensity,
        (active_groups, angle_points),
    )
    right_mean, right_first, right_second = _broadcast_realizable_p2(
        "right_exterior_intensity",
        right_exterior_mean_intensity,
        right_exterior_first_moment_intensity,
        right_exterior_second_moment_intensity,
        (active_groups, angle_points),
    )
    if periodic_spatial_boundary and any(
        np.any(values != 0.0)
        for values in (left_mean, left_first, left_second, right_mean, right_first, right_second)
    ):
        raise PhysicalDomainError(
    ...
    transform = lorentz_ray_transform(mu, angle_weight, beta)
    extinction_mean = absorption_mean + scattering_mean
    extinction_first = absorption_first + scattering_first
    extinction_second = absorption_second + scattering_second
    extinction_state = limit_nonnegative_frequency_group_p2(
        extinction_mean, extinction_first, extinction_second
    )
    if extinction_state.limited_group_count != 0:
        raise ArithmeticError("sum of realizable P2 extinctions became non-realizable")
    angle_shape = (collision_groups, angle_points, depth_points)
    lab_extinction = lorentz_remap_comoving_group_p2_extinction_to_lab(
        np.broadcast_to(extinction_mean[:, None, :], angle_shape),
        np.broadcast_to(extinction_first[:, None, :], angle_shape),
        np.broadcast_to(extinction_second[:, None, :], angle_shape),
    ...
    initial_nodes = frequency_group_p2_gauss_node_values(
        initial_mean, initial_first, initial_second
    )
    left_nodes = frequency_group_p2_gauss_node_values(left_mean, left_first, left_second)
    right_nodes = frequency_group_p2_gauss_node_values(
        right_mean, right_first, right_second
    )
    lab_extinction_nodes = frequency_group_p2_gauss_node_values(
        lab_extinction.mean_density,
        lab_extinction.first_moment_density,
        lab_extinction.second_moment_density,
    )
    product_node, product_weight_raw = np.polynomial.legendre.leggauss(4)
    product_weight = 0.5 * product_weight_raw
    thermal_product_nodes = frequency_group_p2_values_at_normalized_nodes(
        thermal_mean, thermal_first, thermal_second, product_node
    )
    scattering_product_nodes = frequency_group_p2_values_at_normalized_nodes(
        scattering_mean, scattering_first, scattering_second, product_node
    )

    current_mean = np.array(initial_mean, copy=True)
    current_first = np.array(initial_first, copy=True)
    current_second = np.array(initial_second, copy=True)
    final_change = np.inf
    for iteration in range(1, maximum_iterations + 1):
        iteration_limiter = 0
        outer_mean = np.array(outer_mean_template, copy=True)
        outer_first = np.array(outer_first_template, copy=True)
        outer_second = np.array(outer_second_template, copy=True)
        outer_mean[active_slice] = current_mean
        outer_first[active_slice] = current_first
        outer_second[active_slice] = current_second
        comoving = comoving_group_p2_radiation(
            outer_mean,
            outer_first,
            outer_second,
        ...
        comoving_mean_product_nodes = frequency_group_p2_values_at_normalized_nodes(
            comoving.mean_intensity_density,
            comoving.mean_intensity_first_moment_density,
            comoving.mean_intensity_second_moment_density,
            product_node,
        )
        # 中文：四点 Gauss 规则精确投影散射系数与角平均强度的 P2 乘积。
        emissivity = _project_p2_product_nodes(
            thermal_product_nodes
            + scattering_product_nodes * comoving_mean_product_nodes,
            product_node,
            product_weight,
        )
        iteration_limiter, maximum_ratio = _record_limiter(
            iteration_limiter, maximum_ratio, emissivity
        )
        lab_emissivity = lorentz_remap_comoving_group_p2_emissivity_to_lab(
            np.broadcast_to(emissivity.mean_density[:, None, :], angle_shape),
            np.broadcast_to(emissivity.first_moment_density[:, None, :], angle_shape),
            np.broadcast_to(emissivity.second_moment_density[:, None, :], angle_shape),
            stencil.comoving_collision_edge_hz,
            stencil.active_lab_edge_hz,
            transform.doppler_lab_to_comoving,
        )
        ...
        lab_emissivity_nodes = frequency_group_p2_gauss_node_values(
            lab_emissivity.mean_density,
            lab_emissivity.first_moment_density,
            lab_emissivity.second_moment_density,
        )
        ...
        ).reshape(active_groups, 3, angle_points, depth_points)
        updated = frequency_group_p2_from_gauss_node_values(updated_nodes)
        iteration_limiter, maximum_ratio = _record_limiter(
        ...
        scale = max(
            ...
            float(np.max(np.abs(current_first))),
            float(np.max(np.abs(updated.second_moment_density))),
            float(np.max(np.abs(current_second))),
        )
        change = max(
            float(np.max(np.abs(updated.mean_density - current_mean))),
            float(np.max(np.abs(updated.first_moment_density - current_first))),
            float(np.max(np.abs(updated.second_moment_density - current_second))),
        )
        final_change = change / scale if scale > 0.0 else change
        current_mean = np.array(updated.mean_density, copy=True)
        current_first = np.array(updated.first_moment_density, copy=True)
        current_second = np.array(updated.second_moment_density, copy=True)
        cumulative_limiter += iteration_limiter
        if final_change <= tolerance:
            break
    else:
        raise ArithmeticError(
            "mixed-frame P2 ALE source iteration did not converge: "
            f"iterations={maximum_iterations}"
        )

    # 中文：在最终状态重建源项，使残差、能量账本和四维力使用同一状态。
    ...
    emissivity = _project_p2_product_nodes(
        thermal_product_nodes + scattering_product_nodes * comoving_mean_product_nodes,
        product_node,
        product_weight,
    )
    final_limiter, maximum_ratio = _record_limiter(
        final_limiter, maximum_ratio, emissivity
    )
    lab_emissivity = lorentz_remap_comoving_group_p2_emissivity_to_lab(
        np.broadcast_to(emissivity.mean_density[:, None, :], angle_shape),
        np.broadcast_to(emissivity.first_moment_density[:, None, :], angle_shape),
    ...
    final_nodes = frequency_group_p2_gauss_node_values(
        current_mean, current_first, current_second
    )
    lab_emissivity_nodes = frequency_group_p2_gauss_node_values(
        lab_emissivity.mean_density,
        lab_emissivity.first_moment_density,
    ...
    relative_residual = np.max(
        relative_node_residual.reshape(active_groups, 3), axis=1
    )

    lab_angle_mean = 0.5 * np.einsum("m,fmd->fd", angle_weight, current_mean)
    lab_angle_first = 0.5 * np.einsum("m,fmd->fd", angle_weight, current_first)
    lab_angle_second = 0.5 * np.einsum("m,fmd->fd", angle_weight, current_second)
    initial_angle_mean = 0.5 * np.einsum("m,fmd->fd", angle_weight, initial_mean)
    ...
    left_face_nodes = left_face_nodes.reshape(active_groups, 3, angle_points)
    right_face_nodes = right_face_nodes.reshape(active_groups, 3, angle_points)
    left_face = frequency_group_p2_from_gauss_node_values(left_face_nodes)
    right_face = frequency_group_p2_from_gauss_node_values(right_face_nodes)
    final_limiter, maximum_ratio = _record_limiter(
    ...
    cumulative_limiter += left_face.limited_group_count + right_face.limited_group_count
    ...
    material_heating = -2.0 * np.pi * np.einsum(
        "g,m,fgmd,d->f",
        _P2_NODE_WEIGHT,
        angle_weight,
        lab_collision_nodes,
    ...
    comoving_angle_nodes = frequency_group_p2_gauss_node_values(
        comoving.angle_mean_density,
        comoving.angle_first_moment_density,
        comoving.angle_second_moment_density,
    )
    extinction_comoving_nodes = frequency_group_p2_gauss_node_values(
        extinction_mean, extinction_first, extinction_second
    )
    emissivity_comoving_nodes = frequency_group_p2_gauss_node_values(
        emissivity.mean_density,
        emissivity.first_moment_density,
        emissivity.second_moment_density,
    )
    comoving_collision_nodes = (
        emissivity_comoving_nodes[:, :, None, :]
    ...
    minimum_reconstructed = float(
        np.min(_frequency_group_p2_minimum(current_mean, current_first, current_second))
    )
    ...
    if not all(np.all(np.isfinite(array)) for array in arrays):
        raise ArithmeticError("mixed-frame P2 ALE diagnostics became non-finite")
    return MixedFrameALEP2Step(
        final_lab_mean_intensity_density=_readonly(current_mean),
        final_lab_first_moment_intensity_density=_readonly(current_first),
        final_lab_second_moment_intensity_density=_readonly(current_second),
        final_lab_mean_intensity_angle_average=_readonly(lab_angle_mean),
        final_lab_first_moment_intensity_angle_average=_readonly(lab_angle_first),
        final_lab_second_moment_intensity_angle_average=_readonly(lab_angle_second),
        ...
        final_comoving_angle_second_moment_intensity_density=(
            comoving.angle_second_moment_density
        ),
        ...
        final_comoving_mean_second_moment_intensity_density=(
            comoving.mean_intensity_second_moment_density
        ),
        ...
        right_ale_face_second_moment_intensity=right_face.second_moment_density,
        ...
        radiation_source_energy_comoving_erg_s_cm3=_readonly(comoving_source_energy),
        radiation_source_momentum_comoving_dyn_cm3=_readonly(comoving_source_momentum),
