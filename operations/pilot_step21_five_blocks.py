"""Joint 22:27 and 46:51 cores; original 77577 x20 map replay gates Krylov."""
...
def worker(out, label, index):
    ...
    pipeline.write_json(out/f'control-block{index:02d}-replay.json', dict(core_groups=[core.start, core.stop], replay=replay, gmres_started=False))
    del archived
    ...
    result['checks'].update(boundary_not_worse=new_boundary <= raw_boundary,
                            ...
                            memory_below_24gib=rss < PER_WORKER_MIB,
...
def prepare(out):
    ...
    review=pipeline.read(ev/'20260927-convex-joint-complete-review.json')
    if not review['final_summary_present'] or review['windows']['10']['window']['passed']:
        raise RuntimeError('require audited convex window failure')
    ...
    claims+=[pipeline.claim(ev/'20260927-convex-joint-complete-review.json')]+list(record.values())+prior['code']+[s[k] for k in ('outer_base_material','base_residual','physical_old_time_level')]+[pipeline.claim(ranking)]
    claims += [c for c in cfg['sources'] if not c['path'].endswith('.dat')]
    claims=list({(c['path'],c['sha256']):c for c in claims}.values())
    localization=ev/'20260927-convex-feedback-drift-localization.json'
    loc=pipeline.read(localization)
    if loc['component_comparisons']['final_vs_previous']['mass_norm_over_r20']<=.001:
        raise RuntimeError('require unresolved mass-weighted drift')
    claims += [pipeline.claim(localization)]
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/pilot_step21_five_blocks.sbatch',
        'tests/test_joint_frequency_block.py','tests/test_five_block_pilot.py','handoff/protocols/step21-five-block-pilot-v1.md')]
    reused.verify(claims+code)
    plan=dict(cases={'control':record},claims=claims,code=code,blocks=list(BLOCKS),workers=2, core_intervals=[list(core_interval(i)) for i in BLOCKS], per_worker_limit_mib=PER_WORKER_MIB, worker_timeout_s=WORKER_SECONDS,
        ...
        limitation='Five-block interiors coupled, outer halos frozen; no global/material convergence. Same 77577 input as 77648 and 77817 isolates support size.')
