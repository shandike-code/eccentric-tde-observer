"""Prepare one further feedback pair from the settled mixed-state run."""
...
SOURCE = 'outputs/hpc/cross-mixture-feedback-20260919'
...
def main():
    ...
    try:
        ...
        snap.read(source+'/feedback-round1/feedback_summary.json')
        ...
        cfg.update(run=args.run,workers=args.workers,maximum_maps=2,feedback_every=2,
                   ...
                   extension_of=source,extension_purpose='one two-map followup to compare heating stability and all material contraction gates with the preceding mixed-state pair',
        ...
        cfg['sources']=list(cfg['sources'])+[copied['source'],copied['destination'],
            ...
            pipeline.claim(ROOT/'operations/mixed_followup_precision.sbatch'),seed,
            pipeline.claim(ROOT/source/'feedback-round1/feedback_summary.json')]
        declaration={'environment':pipeline.environment(),'history':'settled mixed-state followup','source_run':source,
                     'source_seed':seed,'source_settled_state':pipeline.claim(ROOT/source/'state.json'),'identical_other_material':pipeline.claim(ROOT/other/'trial_material.npz'),
                     'maximum_new_maps':2,'feedback_every':2,'workers':args.workers,
                     'purpose':'measure whether the previous mixed-state heating and material gate failures persist; no automatic extension beyond this pair',
                     'source_feedback_summary':pipeline.claim(ROOT/source/'feedback-round1/feedback_summary.json'),
