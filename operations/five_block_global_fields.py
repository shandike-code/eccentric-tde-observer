...
def checks(report,original,full,half):
    ...
    result['full_l2_benefit']=report['fixed_scale_l2_ratios'][1]<=.8
    return result

CENTERS=(24,48)
SELECTED=(22,23,24,25,26,46,47,48,49,50)
...
def intervals(centers, groups):
    ...
    ranges={i:((i-2)*128,(i+3)*128) for i in centers}
    occupied=set()
...
def write_candidates(source,replacements,full,half,shape,*,centers=CENTERS,checkpoint=lambda:None):
    ...
    for i,p in replacements.items():
        ...
        if cores[i].shape!=(640,*shape[1:]):raise ValueError('joint core shape')
        physical(cores[i])
