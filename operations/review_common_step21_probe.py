...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==20 and root['status']=='half_rejected_requires_direction_review'
    ...
    for case,child,n in (('control','control',4),('half04','half',4),('half08','half',8)):
        ...
        if case=='control':
            ...
            assert cross['passed']==root['cases']['half'][str(n)]['fresh_baseline_comparison']['passed']
            assert root['cases']['half'][str(n)]['pair_supported']==(not failed and cross['passed'])
    ...
    for name in ('pair04','pair08'):
        prior_vectors={e:arrays(reference.parent/name/(e+'_response.npz'))['residual'] for e in ('previous','final')}
    ...
    fig.suptitle('Step21 half amplitude: maximum-cell contraction still fails');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
