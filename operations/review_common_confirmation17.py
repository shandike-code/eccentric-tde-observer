"""Independent six-endpoint review of baseline and same-material confirmations."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==16 and root['status']=='confirmed_requires_mac_review'
    ...
    for case in ('control','confirm1','confirm2'):
        ...
        assert float(t['relaxation'])==(0 if case=='control' else 1/64)
        ...
        if case=='control':
            ...
            frozen=arrays(reference/'trial_material.npz');assert set(t)==set(frozen) and all(np.array_equal(t[k],frozen[k]) for k in t)
        ...
        assert not post['material_step_promoted'] and post['accepted_outer_steps_remain']==16
    ...
    # The new zero control retains accepted x16 exactly, not its response target.
    ...
    prior_vectors={e:arrays(reference.parent/'pair08'/(e+'_response.npz'))['residual'] for e in ('previous','final')}
    ...
    assert seeds['control']['sha256']==read(prior_confirmed/'state.json')['current_sha256']
    assert seeds['confirm1']==read(reference.parent/'endpoints-map08/manifest.json')['endpoints']['mapped_final']
    for case in ('control','confirm1','confirm2'):
    ...
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_confirmation_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':16,'cross_platform_decode_rtol':8*np.finfo(float).eps}
