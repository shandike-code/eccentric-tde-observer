"""One original-operator map of the bounded four-pair half-step prediction."""
...
from operations.scan_half_four_map_subspace import explicit_pairs,SOURCES as PINNED_SOURCES
from operations.half_four_map_candidate import verify_scan,write_candidate,full_field_error

SCAN='outputs/hpc/half-four-map-scan-20260920'
SOURCES=tuple(x[0] for x in PINNED_SOURCES)
PREDICTION_SHA='0cdcd1b720663f1a6e2bbafdbb2b44d4432dacee8dc7afbfbf6d6c61f3a1f5c6'
DECLARATION_SHA='49d5dc124aab3fe0d995621e95153abe6c16b55500069efdf418fdc18c74c6cb'
MANIFEST_SHA='e56a9b56c051271778be4aecb46163bade1d55d74c88f68ea5fb2c0e210d5e7c'
CODE=tuple('operations/'+name for name in (
    'validate_half_four_map.py','validate_half_four_map.sbatch','half_four_map_candidate.py',
    'scan_half_four_map_subspace.py','scan_half_four_map_subspace.sbatch',
    'validate_half_history_candidate.py','validate_cross_history_candidate.py',
    'half_history_basis.py','radiation_history_diagnostic.py','material_direction_diagnostic.py',
    'scan_cross_history_direction.py','scan_multihistory_subspace.py','scan_positive_subspace.py',
    'prepare_extension_run.py','prepare_encoded_backtrack.py','review_small_step_evidence.py'))+(
...
def main():
    ...
    try:
        ...
        prediction=snap.read(SCAN+'/prediction.json',PREDICTION_SHA);declaration=snap.read(SCAN+'/declaration.json',DECLARATION_SHA)
        records={r['path']:r for r in snap.read(SCAN+'/snapshot-manifest.json',MANIFEST_SHA)}
        configs=[];trials=[];states=[]
        ...
        for other in trials[1:]:identical_material(trials[0],other)
        trial=trials[0];cfg=deepcopy(configs[0]);pairs=explicit_pairs(states);basis=[c for pair in pairs for c in pair]
        best=min(float(row['residual']) for s in states for row in s['history'][-2:])
        weights=verify_scan(prediction,declaration,pairs,best)
        for c in basis:
        ...
        plan={'environment':pipeline.environment(),'pairs':pairs,'basis':basis,'effective_weights':weights.tolist(),'prediction':prediction,
              'scan':SCAN,'maximum_new_maps':1,'maximum_candidate_writes':1,'workers':16,
              'formula':'candidate=sum(w_i X_i); prediction=sum(w_i Y_i); exact scan order',
        ...
        candidate=out/'half_four_map_candidate.dat'
        minimum=write_candidate([ROOT/c['path'] for c in basis],candidate,pipeline.SHAPE,weights)
        for c in basis:
        ...
        cfg.update(run=args.run,workers=16,maximum_maps=1,radiation_threshold=1e-4,seed='warm',warm_seed=seed,
                   extension_of=SOURCES[0],extension_purpose='one actual map validating the fixed-material four-pair subspace candidate')
        cfg['sources']=list(cfg['sources'])+[copied['source'],copied['destination'],seed,
            pipeline.claim(out/'validation_declaration.json')]+[
            pipeline.claim(ROOT/source/'trial_material.npz') for source in SOURCES[1:]]+[
        ...
        error=full_field_error([candidate,ROOT/row['output_path']]+[ROOT/c['path'] for c in basis],
                               pipeline.SHAPE,weights,edges)
        ...
        checks['candidate_reconstruction_exact']=error['candidate_reconstruction_exact']
        for c in basis:
