...
def review(archive,receipt,out,reference,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==15 and root['status']=='confirmed_requires_mac_review'
    ...
    for case in ('control','confirm1','confirm2'):
        ...
        t=arrays(out/case/'trial_material.npz');assert digest(out/case/'trial_material.npz')==p['sources']['trial_material']['sha256'];base=arrays(reference/'outer_base_material.npz');old=arrays(reference/'physical_old_time_level.npz');r=np.load(reference/'base_residual.npy',allow_pickle=False)
    ...
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_confirmation_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':15,'cross_platform_decode_rtol':8*np.finfo(float).eps}
