"""Independent ten-endpoint audit of the thermal/population direction experiment."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,prior_probe,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==20 and root['status']=='direction_budget_complete_requires_review' and root['diagnostic_only']
    results={};residuals={};peaks=[];formal='source_formal_heating_erg_s_cm3'
    fig,axes=plt.subplots(1,3,figsize=(14,4),layout='constrained')
    for case,child,n in (('control','control',4),('thermal04','thermal',4),('thermal08','thermal',8),('population04','population',4),('population08','population',8)):
        folder=out/child/f'pair{n:02d}';p=read(folder/'feedback_protocol.json');s=read(folder/('baseline_summary.json' if case=='control' else 'feedback_summary.json'));post=read(folder/'postcheck.json')
        ...
        assert float(t['relaxation'])==(0 if case=='control' else 1/256)
        ...
        state=read(out/child/'state.json');assert len(state['history'])==(4 if case=='control' else 8) and state['active_map'] is None
        ...
        if case=='control':
            ...
            assert cross['passed']==root['cases'][child][str(n)]['fresh_baseline_comparison']['passed']
            assert root['cases'][child][str(n)]['pair_supported']==(not failed and cross['passed'])
            saved=root['cases'][child][str(n)]['fresh_baseline_comparison']['ratios']
        ...
        results[case]={'endpoints':ends,'comparison':s['comparison'],'all_16_pair_gates':case!='control' and all(s['gate_checks'].values()),'gate_checks':s['gate_checks']};residuals[case]=vectors
    ...
    # 新基态只能参与比较；固定r20和两轮候选必须保持同一物质。
    for name in ('pair04','pair08'):
        prior_vectors={e:arrays(prior_probe/'half'/name/(e+'_response.npz'))['residual'] for e in ('previous','final')}
        cross=cross_comparisons(prior_vectors,residuals['control'],mass)
        saved=root['cases']['control']['4']['prior_rejected_candidates'][name]
        assert not cross['passed'] and not saved['passed']
        for e,norms in cross['ratios'].items():
            for k,v in norms.items():np.testing.assert_allclose(v,saved['ratios'][e][k],rtol=8*np.finfo(float).eps,atol=0)
    declaration=read(out/'declaration.json')
    assert declaration['seed']['sha256']==read(prior_probe/'control/state.json')['history'][-1]['output_sha256']
    assert read(out/'control/state.json')['history'][0]['input_sha256']==declaration['seed']['sha256']
    for child in ('thermal','population'):
        assert read(out/child/'state.json')['history'][0]['input_sha256']==read(out/'control/state.json')['history'][-1]['output_sha256']
    ...
    drift={child:asdict(encoded_residual_norms(residuals[child+'08']['final']-residuals[child+'04']['final'],mass)) for child in ('thermal','population')}
    ...
    fig.suptitle('Step21 direction diagnostics: original gates unchanged');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_map4_to_map8_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':20,'cross_platform_decode_rtol':8*np.finfo(float).eps}
    output.with_suffix('.json').write_text(json.dumps(record,indent=2,allow_nan=False)+'\n');print(json.dumps(record,indent=2))


def main():
    p=argparse.ArgumentParser()
    for k in ('archive','receipt','received','reference','physical-old','prior-confirmed','prior-probe','output'):p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();review(a.archive,a.receipt,a.received,a.reference,a.physical_old,a.prior_confirmed,a.prior_probe,a.output)
