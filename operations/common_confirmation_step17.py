"""Fresh baseline and two same-trial confirmations of the supported step17."""
import argparse
...
from operations.common_outer_step17 import verify_step17_pair
...
PRIOR='outputs/hpc/common-step17-20260923'
CONTROL='outputs/hpc/common-confirmation-20260923/confirm2'


def validate_base_against_accepted(base,accepted):
    # 元数据可由第16步试探重置为第17步零控制，物理物质和encoded本身不可改。
    for k in ('encoded_state','temperature_k','hydrogen_fraction','helium_fraction','specific_material_energy_erg_g',
        ...
        if not np.array_equal(base[k],accepted[k]):raise RuntimeError('zero control not accepted x16: '+k)
...
def zero_feedback(folder,p,path,history):
    ...
    verify_step17_pair(folder,p,result)
    return all(gates.values())



def prepare(out):
    review_path=ROOT/'handoff/evidence/20260924-common-step17-review.json';review=pipeline.read(review_path)
    if review['accepted_outer_steps']!=16 or not review['pairs']['pair08']['all_16_pair_gates']:raise RuntimeError('Mac candidate audit missing')
    ...
    terminal=pipeline.read(ROOT/'handoff/evidence/20260924-common-step17-76204-terminal.json')
    if terminal['job_id']!=76204 or terminal['state']!='COMPLETED':raise RuntimeError('source scheduler failure')
    ...
    claims=[pipeline.claim(review_path),pipeline.claim(ROOT/'handoff/evidence/20260924-common-step17-76204-terminal.json')]
    for relative in ('maps/trial_material.npz','maps/config.json','maps/state.json','inputs/outer_base_material.npz',
                     'inputs/base_residual.npy','inputs/feedback_reference.json','pair08/feedback_protocol.json',
                     'pair08/feedback_summary.json','pair08/previous_response.npz','pair08/final_response.npz','endpoints-map08/manifest.json'):
    ...
    reference=pipeline.read(ROOT/PRIOR/'inputs/feedback_reference.json');sources=reference['sources']
    ...
    fresh.bridge.validate_identity(trial,base,r,old);validate_zero(base,base,r,old)
    accepted=load_arrays(ROOT/CONTROL/'trial_material.npz')
    # 仅零控制的迭代元数据改变；真正进入native的物理物质必须等于已接受x16。
    ...
    acceptance=pipeline.read(ROOT/'handoff/evidence/20260923-accepted-step16.json');reused.verify(acceptance['claims'])
    if acceptance['accepted_outer_steps']!=16:raise RuntimeError('step16 is not accepted')
    claims.append(pipeline.claim(ROOT/'handoff/evidence/20260923-accepted-step16.json'))
    ...
    claims+=list(seeds.values())+[pipeline.claim(ROOT/CONTROL/f) for f in ('state.json','config.json','trial_material.npz')]
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_confirmation_step17.sbatch',
        'tests/test_common_confirmation_step17.py','handoff/protocols/common-confirmation-step17-v1.md')]
    plan={'cases':{'base':{'trial':pipeline.claim(prepared/'trial_material.npz'),'config':pipeline.claim(prepared/'config.json')},
        ...
        'accepted_outer_steps':16,'automatic_promotion':False,'physical_dt_changed':False,'environment':pipeline.environment()}
...
def execute(out):
    ...
    def mark(status,**kw):pipeline.write_json(out/'status.json',dict(status=status,updated_unix=time.time(),accepted_outer_steps=16,new_material_steps=0,**kw))
    mark('preparing')
    try:
        ...
        def evaluate(name,count):
            ...
            with reused.feedback_stop_guard():
                if name=='control':passed=zero_feedback(rd,p,path,rows)
                    ...
                    result=pair.run_pair(path,pipeline.sha256(path));verify_step17_pair(rd,p,result)
        ...
        reused.verify(claims+code);reused.immutable(out/'summary.json',{'status':status,'cases':reports,'accepted_outer_steps':16,'new_material_steps':0})
