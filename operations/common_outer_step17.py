"""Bounded step 17 from the independently confirmed common-domain step 16.

The postcheck body is retained from common_native_feedback (48fbed7 lineage);
only its explicit accepted-step counter changes from 15 to 16.
"""
import argparse
...
SOURCE='outputs/hpc/common-confirmation-20260923'
ACCEPTANCE='handoff/evidence/20260923-accepted-step16.json'
...
def make_candidate(base,residual):
    ...
    codec=GroundStateLogSimplexCodec(len(base['temperature_k']));x=base['encoded_state']+residual/64
    if not ground_state_material_trial_within_trust_region(codec,base['encoded_state'],x,
        ...
        maximum_population_fraction_change=.05):raise ValueError('step17 outside unchanged trust region')
    ...
    t.update(encoded_state=x,relaxation=np.array(1/64));return t


def verify_step17_pair(out,p,result):
    ...
    report={'responses':responses,'gate_checks':result['gate_checks'],'comparison':result['comparison'],
        ...
        'accepted_outer_steps_remain':16,'material_step_promoted':False,'frequency_truncation_convergence_established':False}
    reused.immutable(out/'postcheck.json',report);return report


def prepare(out):
    acceptance=pipeline.read(ROOT/ACCEPTANCE)
    if acceptance['accepted_outer_steps']!=16 or acceptance['coupled_column_converged']:
        raise RuntimeError('invalid step16 acceptance record')
    reused.verify(acceptance['claims'])
    review=pipeline.read(ROOT/'handoff/evidence/20260923-common-confirmation-review.json')
    ...
    # 新base=x16本身，绝不把响应目标G(x16)偷当成已接受物质。
    ...
    claims=[pipeline.claim(ROOT/ACCEPTANCE),pipeline.claim(ROOT/'handoff/evidence/20260923-common-confirmation-review.json')]
    ...
    bridge.validate_identity(trial,base,r,old)
    ...
    cfg=deepcopy(pipeline.read(source/'config.json'));cfg['candidate_relaxation']=1/64
    ...
    reference['outer_iteration']={'index':17,'base_accepted_index':16,'base_is_accepted_trial':True,
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_outer_step17.sbatch',
        'tests/test_common_outer_step17.py','handoff/protocols/common-outer-step17-v1.md')]
    ...
    plan={'cases':{'trial':{'trial':pipeline.claim(inputs/'trial_material.npz'),'config':pipeline.claim(inputs/'config.json')}},
        'seed':seed,'code':code,'claims':claims,'environment':pipeline.environment(),'accepted_outer_steps':16,
        'maximum_new_maps':8,'maximum_feedback_pairs':2,'feedback_after_maps':[4,8],
        'new_step_index':17,'alpha':1/64,'automatic_promotion':False,'physical_dt_changed':False}
...
def execute(out):
    ...
    def mark(status,**kw):pipeline.write_json(out/'status.json',dict(status=status,accepted_outer_steps=16,new_material_steps=0,updated_unix=time.time(),**kw))
    mark('preparing')
    try:
        ...
        with relay_dispatch():
            ...
            for _ in range(8):
                ...
                post=verify_step17_pair(rd,p,result)
                ...
                if report['physical_response_failures']:
                    mark('physical_domain_rejected',after_maps=n);return
        ...
        reused.immutable(out/'summary.json',{'status':terminal,'pairs':reports,'accepted_outer_steps':16,'new_material_steps':0})
