"""Fresh baseline and two same-trial confirmations of the supported half-amplitude step20."""
import argparse
...
from operations.common_outer_step20 import verify_step20_pair
...
PRIOR='outputs/hpc/common-step20-20260924'
CONTROL='outputs/hpc/common-confirmation19-20260924/confirm2'
...
def confirmation_protocol(reference,folder,endpoints,rows,trial_claim,is_control):
    ...
    p['numerical_backtracking']={'alpha':0. if is_control else 1/128,
        'fixed_base_accepted_index':19,'fixed_direction':'r19','physical_time_advanced':False}
    return p


def validate_base_against_accepted(base,accepted):
    # 元数据可由第19步试探重置为第20步零控制，物理物质和encoded本身不可改。
    for k in ('encoded_state','temperature_k','hydrogen_fraction','helium_fraction','specific_material_energy_erg_g',
        ...
        if not np.array_equal(base[k],accepted[k]):raise RuntimeError('zero control not accepted x19: '+k)
...
def zero_feedback(folder,p,path,history):
    ...
    verify_step20_pair(folder,p,result)
    return all(gates.values())



def prepare(out):
    review_path=ROOT/'handoff/evidence/20260924-common-step20-review.json';review=pipeline.read(review_path)
    if review['accepted_outer_steps']!=19 or not review['pairs']['pair08']['all_16_pair_gates']:raise RuntimeError('Mac candidate audit missing')
    ...
    terminal=pipeline.read(ROOT/'handoff/evidence/20260924-step20-76639-terminal.json')
    if terminal['job_id']!=76639 or terminal['state']!='COMPLETED':raise RuntimeError('source scheduler failure')
    ...
    claims=[pipeline.claim(review_path),pipeline.claim(ROOT/'handoff/evidence/20260924-step20-76639-terminal.json')]
    ...
    # 仅零控制的迭代元数据改变；真正进入native的物理物质必须等于已接受x19。
    ...
    acceptance=pipeline.read(ROOT/'handoff/evidence/20260924-accepted-step19.json');reused.verify(acceptance['claims'])
    if acceptance['accepted_outer_steps']!=19:raise RuntimeError('step19 is not accepted')
    claims.append(pipeline.claim(ROOT/'handoff/evidence/20260924-accepted-step19.json'))
    ...
    if not np.array_equal(r,load_arrays(ROOT/CONTROL/'common-feedback/final_response.npz')['residual']):raise RuntimeError('r19 changed')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_confirmation_step20.sbatch',
        'tests/test_common_confirmation_step20.py','handoff/protocols/common-confirmation-step20-v1.md')]
    plan={'cases':{'base':{'trial':pipeline.claim(prepared/'trial_material.npz'),'config':pipeline.claim(prepared/'config.json')},
        ...
        'accepted_outer_steps':19,'automatic_promotion':False,'physical_dt_changed':False,'environment':pipeline.environment()}
...
def execute(out):
    ...
    def mark(status,**kw):pipeline.write_json(out/'status.json',dict(status=status,updated_unix=time.time(),accepted_outer_steps=19,new_material_steps=0,**kw))
    mark('preparing')
    try:
        ...
        def evaluate(name,count):
            ...
            with reused.feedback_stop_guard():
                if name=='control':passed=zero_feedback(rd,p,path,rows)
                    ...
                    result=pair.run_pair(path,pipeline.sha256(path));verify_step20_pair(rd,p,result)
        ...
        reused.verify(claims+code);reused.immutable(out/'summary.json',{'status':status,'cases':reports,'accepted_outer_steps':19,'new_material_steps':0})
