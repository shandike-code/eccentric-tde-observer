"""Independent audit of the rejected map4 and supported map8 step19 pairs."""
import argparse
...
def review(archive,receipt,out,reference,baseline,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==18 and root['status']=='candidate_supported_requires_review'
    ...
    for case in ('pair04','pair08'):
        ...
        assert {k for k,v in s['gate_checks'].items() if not v}==({'inner_noise_resolved_pass'} if case=='pair04' else set())
        assert p['outer_iteration']==dict(index=19,base_accepted_index=18,base_is_accepted_trial=True,physical_time_advanced=False,direction_is_latest_confirmed_response=True)
        ...
        assert not post['material_step_promoted'] and post['accepted_outer_steps_remain']==18
    ...
    fig.suptitle('Step19: map4 rejected; map8 supported; confirmation remains required');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_residual_vector_drift_4_to_8':drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':18}
