"""Independent audit of the rejected map4 and supported map8 step20 pairs."""
import argparse
...
def review(archive,receipt,out,reference,baseline,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==19 and root['status']=='candidate_supported_requires_review'
    ...
    for case in ('pair04','pair08'):
        ...
        if case=='pair08':assert all(s['gate_checks'].values())
        # 每个门从本轮实际数组复算；不把上一候选的失败集合当成本轮预设。
        assert p['outer_iteration']==dict(index=20,base_accepted_index=19,base_is_accepted_trial=True,physical_time_advanced=False,direction_is_latest_confirmed_response=True)
    ...
    for case,v in residuals.items():
        ...
        if case=='pair08':assert decision['pair_supported']
    ...
    fig.suptitle('Step20: map4 rejected; map8 supported; confirmation remains required');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_residual_vector_drift_4_to_8':drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':19}
