"""Numbered copy of the reviewed confirmation/one-step contract.

Preparation bodies retain the reviewed lineage checks of accepted_step_confirmation
and second_outer_step, with an explicit source and index contract. Old modules
...
def declare(out):
    ...
    if p.exists():
        ...
        if (plan['source']!=SOURCE or plan['accepted_outer_step_index']!=ACCEPTED_INDEX
            or plan['formal_source']!=FORMAL_SOURCE or plan['maximum_maps']!=MAXIMUM_MAPS or plan['maximum_pairs']!=MAXIMUM_PAIRS):
    ...
    if source['status']!='formal_acceptance_requires_review' or source['candidate']!='full':
        raise RuntimeError('expected accepted full outer step')
    ...
    plan={'source':SOURCE,'accepted_outer_step_index':ACCEPTED_INDEX,'formal_source':FORMAL_SOURCE,'pinned':code+claims,'environment':pipeline.environment(),
          ...
          'next_outer_step_index':ACCEPTED_INDEX+1,'physical_time_advanced':False,'automatic_resubmit':False,
...
def run_confirmation(folder):
    ...
    try:
        # 同态确认保留该接受步原有分母；只有后续新外层步才更新基态。
        with stage_protocols(ACCEPTED_INDEX,FORMAL_SOURCE):
...
def run_next(folder,confirmation_root):
    ...
    try:
        with stage_protocols(ACCEPTED_INDEX+1,str((confirmation_root/'confirm4').relative_to(ROOT))):
...
def execute(out):
    ...
    transition={'confirmation':pipeline.claim(confirmation/'confirmation.json'),
                ...
                'next_outer_step_index':ACCEPTED_INDEX+1,'physical_time_advanced':False}
...
def prepare_confirmation(out):
    ...
    if p.exists():
        plan=pipeline.read(p);reused.verify(plan['pinned'])
        if (plan['source']!=SOURCE or plan['accepted_outer_step_index']!=ACCEPTED_INDEX
            or plan['formal_source']!=FORMAL_SOURCE):raise RuntimeError('confirmation source changed')
        return plan
    ...
    plan={'environment':pipeline.environment(),'cases':cases,'code':code,'seeds':seeds,'limits':confirm.LIMITS,
          ...
          'accepted_outer_step_index':ACCEPTED_INDEX,'source':SOURCE,'formal_source':FORMAL_SOURCE,
...
def prepare_next(out, confirmation_root):
    ...
    if path.exists():
        ...
        if (plan['limits'] != batch.LIMITS or plan['alphas'] != batch.ALPHAS
            or plan['outer_step_index'] != ACCEPTED_INDEX+1
            or plan['confirmation_source'] != source):
    ...
    for label, alpha in [('base', 0.), *batch.ALPHAS.items()]:
        ...
        newcfg.update(candidate_relaxation=alpha, outer_baseline=claims,
            physics_scope=f'nonlinear material step {ACCEPTED_INDEX+1}; confirmed trial as base; unchanged physical old time')
    ...
    plan = {'environment': pipeline.environment(), 'cases': cases, 'code': code,
        ...
        'maximum_maps': 18, 'maximum_pairs': 5, 'outer_step_index': ACCEPTED_INDEX+1,
...
def configure(source, accepted_index):
    """Bind one accepted full eight-map source; never infer index from directory names."""
    global SOURCE, ACCEPTED_INDEX, FORMAL_SOURCE
    if not isinstance(accepted_index,int) or isinstance(accepted_index,bool) or accepted_index<3:
        raise ValueError('invalid accepted outer index')
    folder=pipeline.safe_path(ROOT,source);folder.relative_to(ROOT/'outputs/hpc')
    state=pipeline.read(folder/'full/state.json')
    rd=folder/'full/feedback-round2'
    summary=pipeline.read(rd/'feedback_summary.json')
    confirm.accepted_source(state,summary)
    rec=pipeline.read(rd/'round_summary.json');proto_path=rd/'feedback_protocol.json'
    claim=pipeline.claim(proto_path)
    if claim['sha256']!=rec['protocol_sha256'] or claim['sha256']!=summary['protocol_sha256']:
        raise RuntimeError('configured source protocol hash changed')
    proto=pipeline.read(proto_path)
    if proto['outer_iteration']['index']!=accepted_index or proto['outer_iteration']['physical_time_advanced'] is not False:
        raise RuntimeError('configured source outer index or physical time changed')
    parent=pipeline.read(folder/'status.json')
    if parent['status']!='formal_acceptance_requires_review' or parent['candidate']!='full':
        raise RuntimeError('configured source is not full acceptance')
    if not fresh_contraction(pipeline.read(rd/'fresh_control_comparison.json')):
        raise RuntimeError('configured source fresh control failed')
    # 非线性迭代编号来自实际正式协议；物理旧时间层仍由原协议保护。
    SOURCE=str(folder.relative_to(ROOT));ACCEPTED_INDEX=accepted_index
    FORMAL_SOURCE=proto['outer_iteration']['source']
