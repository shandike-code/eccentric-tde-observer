...
def conditional_sequence(validate,map_one,feedback):
    ...
    return 'tapered_joint_validation_complete_requires_review'
...
def prepare(out):
    ...
    claims=v.audited_inputs(ROOT/'outputs/hpc/step21-joint-line-cost-20260926',
        ev/'20260926-joint-line-cost-review.json',ev/'20260926-joint-line-77905-terminal.json',77905,
        ['declaration.json','prediction.json','status.json'])
    cost=pipeline.read(ev/'20260926-joint-line-cost-review.json')
    if cost['eligible_for_independent_review'] or cost['choice']['best_feasible_l2_ratio']<=.8:
        raise RuntimeError('expected reviewed scalar direction cost rejection')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/validate_step21_tapered_joint.sbatch',
        'tests/test_validate_step21_tapered_joint.py','handoff/protocols/step21-tapered-joint-v1.md')]
    ...
    plan=dict(cases={'control':owned},claims=claims,code=code,selected='tapered-joint-cores23-to25-and47-to49',
        source_pair=[inp,mapped],source_row=state['history'][-1],replacements=replacements,gates=fields.GATES,taper=fields.TAPER,
    ...
    try:
        ...
        with fixed.relay_dispatch():
            ...
            def feedback(n):
                ...
                p['sources']['tapered_joint_validation']=pipeline.claim(folder/'validation.json')
