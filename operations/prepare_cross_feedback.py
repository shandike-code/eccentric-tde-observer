"""Prepare a bounded feedback pair from the fully validated cross-history output."""
...
SOURCE = 'outputs/hpc/cross-history-map-validation-20260919'
...
def validated_cross_seed(state, status, result):
    if (result.get('full_intensity_prediction_error_evaluated') is not True
        or result.get('full_field_error', {}).get('prediction_error_resolved') is not True):
        raise ValueError('cross-history seed requires a resolved full-field prediction check')
    return validated_seed(state, status, result)
...
def main():
    ...
    try:
        ...
        cfg.update(run=args.run,workers=args.workers,maximum_maps=2,feedback_every=2,
                   ...
                   extension_of=source,extension_purpose='two-map feedback diagnostic from a cross-history output validated against its full-field prediction',
        ...
        cfg['sources']=list(cfg['sources'])+[copied['source'],copied['destination'],
            ...
            pipeline.claim(ROOT/'operations/cross_feedback_precision.sbatch'),seed,
            pipeline.claim(ROOT/source/'validation_result.json'),pipeline.claim(ROOT/source/'validation_status.json')]
        declaration={'environment':pipeline.environment(),'history':'validated cross-history mixture','source_run':source,
