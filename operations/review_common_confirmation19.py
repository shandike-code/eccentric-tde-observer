"""Independent step19 six-endpoint audit against fixed x18 and r18."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,candidate_root,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==18 and root['status']=='confirmed_requires_mac_review'
    ...
    for case in ('control','confirm1','confirm2'):
        ...
        assert p['numerical_backtracking']==dict(alpha=0. if case=='control' else 1/128,fixed_base_accepted_index=18,fixed_direction='r18',physical_time_advanced=False)
    ...
    # The new zero control retains accepted x18 exactly, not its response target.
    ...
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_confirmation_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':18,'cross_platform_decode_rtol':8*np.finfo(float).eps}
