"""Independent audit of the rejected map4 and supported map8 step17 pairs."""
import argparse
...
def review(archive,receipt,out,reference,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==16 and root['status']=='candidate_supported_requires_review'
    ...
    for case in ('pair04','pair08'):
        ...
        assert set(s['gate_checks'])==REQUIRED_GATES and all(s['gate_checks'].values())==(case=='pair08') and s['decision']['finite_trial_accepted_as_one_nonlinear_step']==(case=='pair08')
    ...
    prior=reference.parent.parent/'common-confirmation-76075-received/confirm2'
    ...
    fig.suptitle('Step17: map4 rejected; map8 supported; confirmation remains required');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_residual_vector_drift_4_to_8':drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':16}
