"""Independent audit of both mass-norm-rejected fixed-x18 precision pairs."""
import argparse
...
def review(archive,receipt,out,reference,inputs,baseline,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==17 and root['status']=='budget_complete_not_accepted'
    assert digest(out/'maps/trial_material.npz')==digest(inputs/'trial_material.npz')
    assert read(out/'maps/state.json')['history'][0]['input_sha256']==read(inputs.parent/'endpoints-map08/manifest.json')['endpoints']['mapped_final']['sha256']
    ...
    for case in ('pair04','pair08'):
        ...
        for key in ('trial_material','outer_base_material','physical_old_time_level','base_residual'):
            c=p['sources'][key];f=(out/'maps/trial_material.npz' if key=='trial_material' else (reference if key=='physical_old_time_level' else inputs)/(key+Path(c['path']).suffix));assert f.stat().st_size==c['size_bytes'] and digest(f)==c['sha256']
        t=arrays(inputs/'trial_material.npz');base=arrays(inputs/'outer_base_material.npz');old=arrays(reference/'physical_old_time_level.npz');r=np.load(inputs/'base_residual.npy',allow_pickle=False)
        ...
        assert float(t['relaxation'])==1/64 and float(base['relaxation'])==0 and np.array_equal(base['base_encoded_state'],base['encoded_state'])
        assert np.array_equal(base['base_residual'],r)
        codec=GroundStateLogSimplexCodec(128);decoded=codec.decode(t['encoded_state'])
        for key in ('temperature_k','hydrogen_fraction','helium_fraction'):np.testing.assert_allclose(t[key],getattr(decoded,key),rtol=8*np.finfo(float).eps,atol=0)
        ...
        assert set(s['gate_checks'])==REQUIRED_GATES and {k for k,v in s['gate_checks'].items() if not v}=={'candidate_mass_weighted_contraction_pass'} and not s['decision']['finite_trial_accepted_as_one_nonlinear_step']
    ...
    prior=baseline
    prior_trial=arrays(prior/'trial_material.npz');assert np.array_equal(base['encoded_state'],prior_trial['encoded_state'])
    base_vectors={e:arrays(prior/'common-feedback'/(e+'_response.npz'))['residual'] for e in ('previous','final')}
    assert np.array_equal(np.load(inputs/'base_residual.npy',allow_pickle=False),base_vectors['final'])
    ...
    fig.suptitle('Fixed x18: both pairs reject mass-weighted contraction; no acceptance');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_residual_vector_drift_4_to_8':drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':17}
    output.with_suffix('.json').write_text(json.dumps(record,indent=2,allow_nan=False)+'\n');print(json.dumps(record,indent=2))


def main():
    p=argparse.ArgumentParser()
    for k in ('archive','receipt','received','reference','inputs','baseline','output'):p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();review(a.archive,a.receipt,a.received,a.reference,a.inputs,a.baseline,a.output)
