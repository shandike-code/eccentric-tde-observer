"""Bounded step-twelve amplitude test from the audited eleventh confirmation.

Only numerical relaxation changes: 1/64, then 1/128 if not accepted.
...
SOURCE = 'outputs/hpc/outer-steps9to11-20260922/step11/next-step'
CONFIRMATION = 'outputs/hpc/outer-steps9to11-20260922/final-confirmation'
# 只改变非线性物质迭代的数值位移；旧物理时间层与 dt 不变。
ALPHAS = {'full': 1/64, 'half': 1/128}
LIMITS = {'control': 2, 'full': 8, 'half': 8}
PREFLIGHT = 'outputs/hpc/step12-amplitude-preflight-20260922/result.json'
...
def require_source():
    stage.configure(SOURCE, 11)
    if not stage.confirmation_passed(ROOT/CONFIRMATION):
        raise RuntimeError('eleventh precision confirmation did not pass')
