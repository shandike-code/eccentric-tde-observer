"""Prepare two new maps and a feedback pair after a successful affine map validation."""
...
SOURCES={
    'base':'outputs/hpc/affine-base-map-validation-20260919',
    'old':'outputs/hpc/affine-old-map-validation-20260919',
}


def validated_seed(state, status, result):
    """Require a completed real map, not a prediction or a partially mapped seed."""
    if (status.get('status') != 'complete' or status.get('new_maps') != 1
        or status.get('extrapolation_validated') is not True
        or result.get('extrapolation_validated') is not True
        or not result.get('validation_checks')
        or not all(value is True for value in result['validation_checks'].values())
        or state.get('status') != 'radiation' or state.get('active_map')
        or state.get('pending_feedback') or len(state.get('history', [])) != 1):
        raise ValueError('source validation must be complete and successful')
    row = state['history'][0]
    if (row != result['actual_map']
        or row['input_sha256'] != result['candidate']['sha256']
        or state['slots'][state['current_slot']] != row['output_path']
        or state['current_sha256'] != row['output_sha256']):
        raise ValueError('validation does not identify the latest mapped output')
    # 用真实映射输出作新种子；不把外推输入或旧 history 拼进新反馈对。
    return {'path': row['output_path'], 'sha256': row['output_sha256'],
...
def main():
    ...
    try:
        ...
        seed=validated_seed(state,status,result)
        ...
        other=SOURCES['old' if args.history=='base' else 'base']
        other_state=snap.read(other+'/state.json')
        validated_seed(other_state,snap.read(other+'/validation_status.json'),
                       snap.read(other+'/validation_result.json'))
        ...
        cfg.update(run=args.run,workers=args.workers,maximum_maps=2,feedback_every=2,
                   ...
                   extension_of=source,extension_purpose='two-map feedback diagnostic after independently validated affine radiation acceleration',
        ...
        cfg['sources']=list(cfg['sources'])+[copied['source'],copied['destination'],
            ...
            pipeline.claim(ROOT/'operations/affine_feedback_precision.sbatch'),seed,
        ...
        declaration={'environment':pipeline.environment(),'history':args.history,'source_run':source,
                     ...
                     'purpose':'measure whether same-material feedback histories approach each other after acceleration',
