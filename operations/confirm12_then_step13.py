"""Confirm accepted step twelve (1/64), then conditionally take one thirteenth step.
...
ACCEPTED_ALPHA=1/64
FORMAL_SOURCE=None
NEXT_ALPHAS={'full':1/64,'half':1/128}


def validate_accepted_material(base,trial):
    # 数值位移明确为1/64；同态确认不推进物理时间，也不更新原正式分母。
...
def execute(out):
    ...
    if (d.get('outer_step_index')!=12 or d.get('alphas')!=NEXT_ALPHAS
