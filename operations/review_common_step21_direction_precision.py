"""Independent twelve-endpoint audit of fixed-trial radiation precision."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,prior_directions,output):
    ...
    for case,child,n in (('control04','control',4),('control08','control',8),('thermal04','thermal',4),('thermal08','thermal',8),('population04','population',4),('population08','population',8)):
        ...
        state=read(out/child/'state.json');assert len(state['history'])==8 and state['active_map'] is None
    ...
    # 每个候选原样继续自身场，源trial必须与已审76905归档逐字节相同。
    ...
    for child in ('control','thermal','population'):
        ...
        assert declaration['seeds'][child]['sha256']==read(prior_directions/child/'state.json')['history'][-1]['output_sha256']
        assert read(out/child/'state.json')['history'][0]['input_sha256']==declaration['seeds'][child]['sha256']
    ...
    drift={child:asdict(encoded_residual_norms(residuals[child+'08']['final']-residuals[child+'04']['final'],mass)) for child in ('control','thermal','population')}
    ...
    fig.suptitle('Step21 fixed-trial precision: original gates unchanged');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
...
def main():
    ...
    for k in ('archive','receipt','received','reference','physical-old','prior-confirmed','prior-directions','output'):p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();review(a.archive,a.receipt,a.received,a.reference,a.physical_old,a.prior_confirmed,a.prior_directions,a.output)
