"""Joint 23:26 and 47:50 cores; original 77577 x20 map replay gates Krylov."""
...
def worker(out, label, index):
    ...
    base = native.base
    block = joint_block(context['stencil'], context['mu'], context['weight'], context['beta'], 128*(index-1), 128*(index+2))
    if (block.core_group_start, block.core_group_stop) != (128*(index-1), 128*(index+2)):
...
def prepare(out):
    ...
    review=pipeline.read(ev/'20260926-short-validation-complete-review.json')
    if not review['final_summary_present'] or review['windows']['10']['window']['passed']:
        raise RuntimeError('require audited short-step window failure')
    ...
    claims+=[pipeline.claim(ev/'20260926-short-validation-complete-review.json')]+list(record.values())+prior['code']+[s[k] for k in ('outer_base_material','base_residual','physical_old_time_level')]+[pipeline.claim(ranking)]
    claims += [c for c in cfg['sources'] if not c['path'].endswith('.dat')]
    claims=list({(c['path'],c['sha256']):c for c in claims}.values())
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/pilot_step21_joint_blocks.sbatch',
        'tests/test_joint_frequency_block.py','handoff/protocols/step21-joint-block-pilot-v1.md')]
    reused.verify(claims+code)
    plan=dict(cases={'control':record},claims=claims,code=code,blocks=list(BLOCKS),workers=2, core_intervals=[[2944,3328],[6016,6400]], per_worker_limit_mib=16384,
        ...
        limitation='Three-block interiors coupled, outer halos frozen; no global/material convergence. Same 77577 input as 77648 isolates support size.')
...
def execute(out):
    ...
    try:
        ...
        def launch(index):
            ...
            command=[sys.executable,str(ROOT/'operations/native_worker_relay.py'),'--receipt',str(receipt),'--limit-kib',str(16*1024**2),'--',
