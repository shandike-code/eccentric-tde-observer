"""Audit the verified 74751 small bundle and plot its fixed-material evidence."""
import argparse
import hashlib
import json
...
def main():
    ...
    p.add_argument('--output',type=Path,required=True);a=p.parse_args();root=a.received
    ...
    axes[0].plot(labels,[.0005477903334037252]+[result['cases'][k]['formal_comparison']['atomic_heating_volume_l1'] for k in ('confirm2','confirm4')],'o-')
    axes[0].axhline(.001,color='black',lw=.8);axes[0].set(title='Heating pair change',ylabel='Relative L1 change')
    axes[1].plot(labels,[.05347375925998559]+[result['cases'][k]['formal_comparison']['inner_noise_to_trial_signal_l2_ratio'] for k in ('confirm2','confirm4')],'o-')
    ...
    axes[2].axhline(1,color='black',lw=.8);axes[2].set(title='Residual / original baseline');axes[2].legend()
    fig.suptitle('74751: same accepted material, two precision checks; NOT a converged column')
