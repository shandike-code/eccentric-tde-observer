"""Prepare two maps and one formal pair from the validated four-map output."""
...
SOURCE = 'outputs/hpc/half-four-map-validation-20260920'
SOURCE_HASHES = {
    'state.json':'77e949859e24baaf38fee520e28686e433e302ebfc93f57ee5913e3db3c5f984',
    'validation_status.json':'a57a2038fe6449ceba09803360524c50b22278b4f491445bd809769f8d449916',
    'validation_result.json':'8f2d803aba76e5f6b5826bcf533ba8a94a16bc1795a2925a4ff43124b659539c',
    'validation_declaration.json':'3ed1fbe06e0556ef33b34f101776c3bb9695111c813034bfbdbd6c5d1779e150',
}
...
EXPECTED_VALIDATION = {'actual_maximum_norm_improves', 'actual_boundary_spectrum_pass',
                       ...
                       'full_field_prediction_error_resolved', 'candidate_reconstruction_exact'}


def validated_four_map_seed(state, status, result, declaration):
    seed = validated_cross_seed(state, status, result)
    if (set(result['validation_checks']) != EXPECTED_VALIDATION
        or declaration['maximum_new_maps'] != 1 or declaration['maximum_candidate_writes'] != 1
        or declaration['workers'] != 16 or declaration['candidate_clipping'] is not False
        or result['full_field_error']['candidate_reconstruction_exact'] is not True
        or declaration['material_changed'] is not False
...
def main():
    ...
    try:
        source=SOURCE
        state=snap.read(source+'/state.json', SOURCE_HASHES['state.json'])
        status=snap.read(source+'/validation_status.json', SOURCE_HASHES['validation_status.json'])
        result=snap.read(source+'/validation_result.json', SOURCE_HASHES['validation_result.json'])
        validation=snap.read(source+'/validation_declaration.json', SOURCE_HASHES['validation_declaration.json'])
        seed=validated_four_map_seed(state,status,result,validation)
        ...
        cfg.update(run=args.run,workers=args.workers,maximum_maps=2,feedback_every=2,
                   ...
                   extension_of=source,extension_purpose='two-map formal feedback from the original-map-validated four-pair half-step output',
        ...
        cfg['sources']=list(cfg['sources'])+[copied['source'],copied['destination'],
            ...
            pipeline.claim(ROOT/'operations/half_four_map_feedback.sbatch'),
        ...
        declaration={'environment':pipeline.environment(),'history':'validated identical-half-step four-pair radiation combination','source_run':source,
