...
def conditional_sequence(validate,map_one,feedback):
    ...
    return 'convex_joint_validation_complete_requires_review'
...
def prepare(out):
    ...
    scanroot=ROOT/'outputs/hpc/step21-joint-taper-plane-20260927'
    claims=v.audited_inputs(scanroot,ev/'20260927-joint-taper-plane-review.json',ev/'20260927-joint-taper-plane-78031-terminal.json',78031,['declaration.json','gram.json','prediction.json','status.json'])
    audit=pipeline.read(ev/'20260927-joint-taper-plane-review.json');scan=pipeline.read(scanroot/'declaration.json');prediction=pipeline.read(scanroot/'prediction.json')
    last=prediction['rounds'][-1]['result']
    if not audit['eligible_for_independent_review'] or not prediction['eligible_for_independent_review'] or not all(last['checks'].values()) or audit['selected_uv']!=last['uv']:
        raise RuntimeError('audited eligible convex direction required')
    uv=last['uv'];basis=[scan['paths'][i] for i in (0,2,4)]
    claims+=scan['claims']+scan['code']
    ...
    if record['input']!=inp or record['output']!=mapped or scan['paths'][:2]!=[inp,mapped]:raise RuntimeError('source map differs')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/validate_step21_convex_joint.sbatch',
        'tests/test_validate_step21_convex_joint.py','handoff/protocols/step21-convex-joint-v1.md')]
    ...
    plan=dict(cases={'control':owned},claims=claims,code=code,selected='convex-joint-and-tapered-directions',
        source_pair=[inp,mapped],source_row=state['history'][-1],replacements=replacements,gates=fields.GATES,convex_basis=basis,uv=uv,prediction=last,prediction_ratio_absolute_tolerance=1e-6,
...
def execute(out):
    ...
    try:
        ...
        fields.write_candidates([ROOT/c['path'] for c in plan['convex_basis']],plan['uv'],
            seedpath,halfpath,pipeline.SHAPE,checkpoint=reused.checkpoint)
        reused.verify(plan['source_pair']+plan['convex_basis'])
        ...
        with fixed.relay_dispatch():
            ...
            def validate():
                ...
                checks.update(fields.prediction_checks(comparison,plan['prediction']))
            ...
            def feedback(n):
                ...
                p['sources']['convex_joint_validation']=pipeline.claim(folder/'validation.json')
