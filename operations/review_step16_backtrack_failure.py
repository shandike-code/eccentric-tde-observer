...
def review(root, source):
    ...
    assert float(t['relaxation'])==1/64
    ...
    assert np.array_equal(t['encoded_state'],base['encoded_state']+direction/64)
    ...
    state=read(root/'full/state.json');result={'verified_files':len(inventory['files']),'actual_alpha':1/64,'native_identity':True,'rounds':{}}
    for ri in (1,):
    ...
    assert not (root/'full/feedback-round1/feedback_summary.json').exists()
    result.update(round1_material_response_evaluated=False,coupled_column_accepted=False)
    return result
...
def main():
    ...
    r=review(a.received,Path('outputs/hpc/step16-backtrack-20260923'))
