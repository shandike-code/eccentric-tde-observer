"""Taper only the numerical correction; the full-frequency physical map is unchanged."""
from pathlib import Path
import numpy as np
from operations.heating_block_global_fields import chunks,physical
from operations.composite_hybrid_audit import GATES as BASE_GATES, checks as base_checks
from operations.heating_block_global_fields import validate as original_validate
GATES={**BASE_GATES,'full_l2_ratio_max':.8}
TAPER={'kind':'sin_squared_midpoint','groups_per_core':384,'formula':'sin(pi*(j+0.5)/384)^2','free_parameters':0}


def taper_weights():
    return np.sin(np.pi*(np.arange(384,dtype=np.float64)+.5)/384.)**2


def checks(report,original,full,half):
    result=base_checks(report,original,full,half)
    result['full_l2_benefit']=report['fixed_scale_l2_ratios'][1]<=GATES['full_l2_ratio_max']
    return result
    ...
    with temporary[0].open('xb') as f,temporary[1].open('xb') as h:
        for start,(x,) in chunks([source],shape):
            ...
            if which:
                ...
                w=taper_weights()[start-a:start-a+len(x),None,None]
                q=(1.-w)*x+w*cores[i][start-a:start-a+len(x)];m=.5*x+.5*q
            else:q=m=x
