...
from operations import pilot_step21_five_blocks as pilot
from operations import five_block_global_fields as fields

v=wide.validation
ROOT,pipeline,fresh,fixed,reused,driver=v.ROOT,v.pipeline,v.fresh,v.fixed,v.reused,v.driver
StreamState=v.StreamState
SOURCE=pilot.SOURCE
PILOT='outputs/hpc/step21-five-block-pilot-20260927'
LIMIT=10
...
def conditional_sequence(validate,map_one,feedback):
    ...
    return 'five_block_global_validation_complete_requires_review'
...
def prepare(out):
    ...
    claims=v.audited_inputs(local,ev/'20260927-five-block-pilot-metadata-review.json',
        ev/'20260927-five-block-78037-terminal.json',78037,names)
    review=pipeline.read(ev/'20260927-five-block-pilot-metadata-review.json')
    ...
    array_review=ev/'20260927-five-array-independent-review.json'
    ar=pipeline.read(array_review)
    if ar['job_id']!=78038 or ar['verified_frequency_rows']!=1280 or not ar['mac_independent_fsum'] or not ar['school_raw_arrays_scanned']:
        raise RuntimeError('raw array scan and independent reduction required')
    scanroot=ROOT/'outputs/hpc/step21-five-block-array-audit-20260927'
    scan=pipeline.read(scanroot/'declaration.json');scan_summary=pipeline.read(scanroot/'summary.json')
    if scan_summary['status']!='complete_requires_mac_reduction' or not scan_summary['source_unchanged']:
        raise RuntimeError('incomplete source scan')
    claims += scan['claims']+scan['code']+[pipeline.claim(array_review),pipeline.claim(scanroot/'declaration.json'),pipeline.claim(scanroot/'summary.json')]
    replacements={}
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/validate_step21_five_blocks.sbatch',
        'tests/test_validate_step21_five_blocks.py','handoff/protocols/step21-five-block-global-v1.md')]
    ...
    plan=dict(cases={'control':owned},claims=claims,code=code,selected='five-cores22-to26-and46-to50-full',
    ...
    try:
        ...
        with fixed.relay_dispatch():
            ...
            def feedback(n):
                ...
                p['sources']['five_block_global_validation']=pipeline.claim(folder/'validation.json')
