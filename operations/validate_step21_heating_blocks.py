...
def conditional_sequence(validate,map_one,feedback):
    ...
    return 'block_global_validation_complete_requires_review'
...
def prepare(out):
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/validate_step21_heating_blocks.sbatch',
        'tests/test_validate_step21_heating_blocks.py','handoff/protocols/step21-heating-block-global-v1.md')]
    ...
    plan=dict(cases={'control':owned},claims=claims,code=code,selected='blocks24-and48-full',
    ...
    try:
        ...
        with fixed.relay_dispatch():
            ...
            def feedback(n):
                ...
                p['sources']['heating_block_global_validation']=pipeline.claim(folder/'validation.json')
