...
def scan(paths,shape,flux,original_row):
    ...
    for start,(x,y,q,tq) in chunks(paths,shape):
        ...
        if bound<upper:
            ...
            witness=dict(index=[start+i,j,k],raw_hex=float(r[index]).hex(),delta_hex=float(d[index]).hex(),maximum_hex=float(maximum).hex(),upper=bound)
...
def prepare(out):
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/scan_step21_block_short_step.sbatch','tests/test_scan_step21_block_short_step.py','handoff/protocols/step21-block-short-step-scan-v1.md')]
