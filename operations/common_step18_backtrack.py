"""Fresh x17 baseline, then at most two amplitude trials of the fixed r17 direction."""
import argparse
...
PRIOR='outputs/hpc/common-step18-precision-20260924'
BASE='outputs/hpc/common-confirmation17-20260924/confirm2'
SOURCE='outputs/hpc/common-step18-20260924'
ALPHAS={'control':0.,'half':1/128,'quarter':1/256}
LIMITS={'control':4,'half':8,'quarter':8}
...
def amplitude_sequence(evaluate):
    ...
    for name in ('half','quarter'):
        ...
        if result=='supported':return name+'_supported_requires_review'
        if result!='contraction_rejected':return 'stopped_at_'+name
    return 'two_amplitudes_rejected_requires_direction_review'
...
def prepare(out):
    audit_path=ROOT/'handoff/evidence/20260924-common-step18-precision-review.json';audit=pipeline.read(audit_path)
    terminal_path=ROOT/'handoff/evidence/20260924-step18-precision-76405-terminal.json';terminal=pipeline.read(terminal_path)
    if terminal['job_id']!=76405 or terminal['state']!='COMPLETED' or audit['accepted_outer_steps']!=17:raise RuntimeError('source audit missing')
    if set(audit['pairs'])!={'pair04','pair08'}:raise RuntimeError('incomplete source audit')
    for result in audit['pairs'].values():
        if {k for k,v in result['gate_checks'].items() if not v}!={'candidate_mass_weighted_contraction_pass'}:raise RuntimeError('failure mechanism changed')
    ...
    for name in ('maps/trial_material.npz','maps/config.json','pair04/feedback_summary.json','pair04/previous_response.npz','pair04/final_response.npz','pair08/feedback_summary.json','pair08/feedback_protocol.json','pair08/previous_response.npz','pair08/final_response.npz'):
    ...
    if acceptance['accepted_outer_steps']!=17 or acceptance['accepted_target_is_response'] or acceptance['physical_time_advanced']:raise RuntimeError('acceptance identity changed')
    reused.verify(acceptance['claims']);claims.append(pipeline.claim(acceptance_path))
    if not np.array_equal(r,load_arrays(ROOT/BASE/'common-feedback/final_response.npz')['residual']):raise RuntimeError('fixed r17 differs')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_step18_backtrack.sbatch','tests/test_common_step18_backtrack.py','handoff/protocols/common-step18-backtrack-v1.md')]
    plan={'cases':cases,'seed':seed,'claims':claims,'code':code,'alphas':ALPHAS,'limits':LIMITS,'maximum_maps':20,'maximum_feedback_pairs':5,'accepted_outer_steps':17,'automatic_promotion':False,'physical_dt_changed':False,'environment':pipeline.environment()}
    ...
    try:
        ...
        def evaluate(name):
            ...
            for _ in range(LIMITS[name]):
                ...
                p['numerical_backtracking']={'alpha':ALPHAS[name],'fixed_base_accepted_index':17,'fixed_direction':'r17','physical_time_advanced':False}
                ...
                with reused.feedback_stop_guard():
                    ...
                    else:result=pair.run_pair(path,pipeline.sha256(path));verify_step18_pair(rd,p,result)
                ...
                if not report['physical_response_failures']:
                    ...
                    if name=='control':
                        ...
                        report['fresh_control_minus_r17_norms']=asdict(fresh.bridge.encoded_residual_norms(vectors['final']-r,old['cell_mass_g_cm2']))
                ...
                if name=='control':
                    ...
                    if stable:
                        seeds['half']=latest_seed(state);seeds['quarter']=seeds['half']
                        return 'baseline_stable'
                    return 'baseline_unstable'
                if report['pair_supported']:reports[name]=case_reports;return 'supported'
            reports[name]=case_reports
        ...
        reused.verify(plan['claims']+plan['code']);reused.immutable(out/'summary.json',{'status':terminal,'cases':reports,'accepted_outer_steps':17,'new_material_steps':0})
