"""Read-only full-domain Linf-constrained line scan after the 77843 rejection; require 20% predicted L2 gain."""
import argparse,math,os,resource,signal,sys,time
from pathlib import Path
import numpy as np
from operations import validate_step21_joint_blocks as global_run
...
SOURCE='outputs/hpc/step21-joint-block-global-20260926'
BLOCKS=(23,24,25,47,48,49)
CONTROLS=dict(step_safety=.9,maximum_fraction=1.,minimum_l2_gain=.2,linf_roundoff=1e-10,
...
def choose(s0,dd,rd,upper):
    ...
    optimum=-rd/dd;best=min(optimum,upper,1.);t=.9*best
    best_square=s0+2*best*rd+best*best*dd
    if best_square<0:raise ArithmeticError('negative best squared norm')
    square=s0+2*t*rd+t*t*dd
    if square<0:raise ArithmeticError('negative squared norm')
    ratio=math.sqrt(square/s0)
    return dict(fraction=t,best_fraction_without_safety=best,best_feasible_l2_ratio=math.sqrt(best_square/s0),unconstrained_l2_optimum=optimum,predicted_l2_ratio=ratio,l2_cost_pass=ratio<=1-CONTROLS['minimum_l2_gain'],reason='fixed constrained optimum with ten percent step margin')
...
def scan(paths,shape,flux,original_row):
    ...
    if choice['fraction']>0:
        ...
        ratio=math.sqrt(math.fsum(ss)/s0);checks['streamed_l2_cost_pass']=ratio<=1-CONTROLS['minimum_l2_gain']
...
def prepare(out):
    ...
    claims=v.audited_inputs(source,ev/'20260926-joint-global-review.json',ev/'20260926-joint-global-77843-terminal.json',77843,names)
    review=pipeline.read(ev/'20260926-joint-global-review.json');val=pipeline.read(source/'control/validation.json');prior=pipeline.read(source/'declaration.json')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/scan_step21_joint_line_cost.sbatch','tests/test_scan_step21_joint_line_cost.py','handoff/protocols/step21-joint-line-cost-scan-v1.md')]
