"""Independent six-endpoint audit of the fixed-direction half-step experiment."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==17 and root['status']=='half_supported_requires_review'
    results={};residuals={};peaks=[];formal='source_formal_heating_erg_s_cm3'
    fig,axes=plt.subplots(1,3,figsize=(14,4),layout='constrained')
    for case,child,n in (('control','control',4),('half04','half',4),('half08','half',8)):
        ...
        if case=='control':
            ...
            frozen=arrays(out/'inputs/half/trial_material.npz');assert set(t)==set(frozen) and all(np.array_equal(t[k],frozen[k]) for k in t)
        ...
        if case=='control':
            ...
            expected={'last_two_atomic_heating_pass','last_two_direct_heating_pass','last_two_formal_heating_pass'} if case=='half04' else set()
            assert failed==expected and s['decision']['finite_trial_accepted_as_one_nonlinear_step']==(not failed)
            ...
            assert cross['passed']
            saved=root['cases']['half'][str(n)]['fresh_baseline_comparison']['ratios']
        ...
        assert not post['material_step_promoted'] and post['accepted_outer_steps_remain']==17
    ...
    # 新基态只能参与比较；固定r17和两轮候选必须保持同一物质。
    for name in ('pair04','pair08'):
        prior_vectors={e:arrays(reference.parent.parent/'common-step18-precision-76405-received'/name/(e+'_response.npz'))['residual'] for e in ('previous','final')}
    ...
    assert declaration['seed']['sha256']==read(prior_confirmed/'state.json')['history'][-1]['output_sha256']
    assert read(out/'control/state.json')['history'][0]['input_sha256']==declaration['seed']['sha256']
    assert read(out/'half/state.json')['history'][0]['input_sha256']==read(out/'control/state.json')['history'][-1]['output_sha256']
    assert not (out/'quarter').exists()
    for child in ('control','half'):
    ...
    drift=asdict(encoded_residual_norms(residuals['half08']['final']-residuals['half04']['final'],mass))
    ...
    fig.suptitle('Half-step: map4 heating rejected, map8 supported; no step acceptance');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_map4_to_map8_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':17,'cross_platform_decode_rtol':8*np.finfo(float).eps}
    output.with_suffix('.json').write_text(json.dumps(record,indent=2,allow_nan=False)+'\n');print(json.dumps(record,indent=2))


def main():
    p=argparse.ArgumentParser()
    for k in ('archive','receipt','received','reference','physical-old','prior-confirmed','output'):p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();review(a.archive,a.receipt,a.received,a.reference,a.physical_old,a.prior_confirmed,a.output)
