"""CPU-long two-map control of the adjacent candidate from archived base radiation.
...
SOURCE='outputs/hpc/small-step-a078125-cont24-20260918'
ALPHA=0.0078125
MAXIMUM_MAPS=2
WORKERS=16


def common_seed_trial(source, base, baseline_residual):
    """Preserve every candidate field exactly; only radiation initialization differs."""
    # 物质候选逐位不变；与既有小步运行的差别仅是辐射初值及迭代历史。
    ...
    if float(source['relaxation']) != ALPHA:
        raise ValueError('wrong candidate alpha')
    codec=GroundStateLogSimplexCodec(len(source['temperature_k']))
    decoded=codec.decode(source['encoded_state'])
    ...
    for k in ('temperature_k','hydrogen_fraction','helium_fraction','specific_material_energy_erg_g'):
        if not np.array_equal(trial[k],getattr(decoded,k)):
            raise ValueError('candidate decoded field differs')
    return trial
...
def main():
    ...
    try:
        ...
        trial=common_seed_trial(source,base,legacy)
        ...
        for k in ('encoded_state','base_encoded_state','finite_direction','base_residual','relaxation'):
            if not np.array_equal(reread[k],trial[k]):raise RuntimeError('written candidate identity mismatch')
        ...
        for k in ('temperature_k','hydrogen_fraction','helium_fraction'):
            ...
            # 此处记录真实有限物质位移，不把候选与基态不同误判为迁移失败。
        config.update(run=args.run,workers=WORKERS,maximum_maps=MAXIMUM_MAPS,seed='warm',warm_seed=seed,
                      candidate_relaxation=ALPHA,radiation_threshold=1e-4,
                      physics_scope='same finite candidate, common archived radiation seed; diagnostic only',
        ...
        declaration={'scope':'common-seed candidate check; never material acceptance','maximum_new_maps':MAXIMUM_MAPS,'workers':WORKERS,
                     ...
                     'pre_run_check':{'code':'exact existing candidate identity and native input checks',
                                      'logic':'archived I and adjacent existing candidate, two maps and feedbacks, compare with base and half-step control',
        ...
        fp['phase']='common-seed finite candidate feedback audit'
        fp['authorization'].update(accept_material_step=False,common_seed_control=True)
        ...
        result={'scope':'common-seed candidate control only','alpha':ALPHA,'accepted_material_step':False,'history':state['history'],
        ...
        for label in ('previous','final'):
            # 两次反馈分别消费历史 I 和候选物质下的一次映射输出；不能据此接受物质步。
        ...
        result['limitations']=['Two further maps do not establish a true inner error bound.',
            'Previous endpoint uses original base radiation with a different, finite candidate material.',
            'Final endpoint changes only radiation at identical candidate matter; neither is assumed converged.',
            'Archive comparisons include both physical material displacement and radiation iteration effects.',
        ...
        state['status']='common_seed_control_complete';pipeline.write_json(out/'state.json',state)
