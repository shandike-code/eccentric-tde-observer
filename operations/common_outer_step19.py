"""Bounded step 19 from the independently confirmed common-domain step 18.

The postcheck body is retained from common_native_feedback (48fbed7 lineage);
Its explicit accepted-step counter is now 18; historical functions stay frozen.
"""
import argparse
...
SOURCE='outputs/hpc/common-confirmation18-20260924'
ACCEPTANCE='handoff/evidence/20260924-accepted-step18.json'


def validate_acceptance(record,terminal):
    if (record['accepted_outer_steps']!=18 or record['previous_accepted_outer_steps']!=17
        or record['accepted_target_is_response'] or record['coupled_column_converged']
        or record['physical_time_advanced'] or record['job_id']!=76469
        or record['candidate']!=SOURCE+'/confirm2/trial_material.npz'
        or terminal['job_id']!=76469 or terminal['state']!='COMPLETED'):
        raise RuntimeError('invalid independently confirmed step18 provenance')
...
def make_candidate(base,residual):
    ...
    if not ground_state_material_trial_within_trust_region(codec,base['encoded_state'],x,
        ...
        maximum_population_fraction_change=.05):raise ValueError('step19 outside unchanged trust region')
...
def verify_step19_pair(out,p,result):
    ...
    report={'responses':responses,'gate_checks':result['gate_checks'],'comparison':result['comparison'],
        ...
        'accepted_outer_steps_remain':18,'material_step_promoted':False,'frequency_truncation_convergence_established':False}
    reused.immutable(out/'postcheck.json',report);return report


def prepare(out):
    acceptance=pipeline.read(ROOT/ACCEPTANCE)
    terminal_path=ROOT/'handoff/evidence/20260924-confirmation18-76469-terminal.json'
    validate_acceptance(acceptance,pipeline.read(terminal_path))
    reused.verify(acceptance['claims'])
    review=pipeline.read(ROOT/'handoff/evidence/20260924-common-confirmation18-review.json')
    ...
    # 新base=x18本身，绝不把响应目标G(x18)偷当成已接受物质。
    ...
    claims=[pipeline.claim(ROOT/ACCEPTANCE),pipeline.claim(terminal_path),pipeline.claim(ROOT/'handoff/evidence/20260924-common-confirmation18-review.json')]
    ...
    reference['outer_iteration']={'index':19,'base_accepted_index':18,'base_is_accepted_trial':True,
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_outer_step19.sbatch',
        'tests/test_common_outer_step19.py','handoff/protocols/common-outer-step19-v1.md')]
    ...
    plan={'cases':{'trial':{'trial':pipeline.claim(inputs/'trial_material.npz'),'config':pipeline.claim(inputs/'config.json')}},
        'seed':seed,'code':code,'claims':claims,'environment':pipeline.environment(),'accepted_outer_steps':18,
        'maximum_new_maps':8,'maximum_feedback_pairs':2,'feedback_after_maps':[4,8],
        'new_step_index':19,'alpha':1/128,'automatic_promotion':False,'physical_dt_changed':False}
...
def execute(out):
    ...
    def mark(status,**kw):pipeline.write_json(out/'status.json',dict(status=status,accepted_outer_steps=18,new_material_steps=0,updated_unix=time.time(),**kw))
    mark('preparing')
    try:
        ...
        with relay_dispatch():
            ...
            for _ in range(8):
                ...
                post=verify_step19_pair(rd,p,result)
                ...
                if report['physical_response_failures']:
                    ...
                    reused.immutable(out/'summary.json',{'status':terminal,'pairs':reports,'accepted_outer_steps':18,'new_material_steps':0})
