"""Validate the independently audited constrained short radiation step and half."""
import argparse
...
from operations import scan_step21_block_short_step_v2 as scan

v=wide.validation
ROOT,pipeline,fresh,fixed,reused,driver=v.ROOT,v.pipeline,v.fresh,v.fixed,v.reused,v.driver
StreamState=v.StreamState
SOURCE=pilot.SOURCE
PILOT='outputs/hpc/step21-heating-block-pilot-20260926'
SCAN='outputs/hpc/step21-block-short-step-v2-20260926'
LIMIT=10
...
def conditional_sequence(validate,map_one,feedback):
    ...
    return 'short_step_validation_complete_requires_review'


def reviewed_fraction(declaration,prediction,audit,source_pair):
    if declaration['paths'][:2]!=source_pair or declaration['controls']!=scan.CONTROLS:
        raise RuntimeError('audited source or scan controls differ')
    if not prediction['eligible_for_independent_review'] or not audit['eligible_for_independent_review']:
        raise RuntimeError('short step not eligible')
    if prediction['checks']!=audit['checks'] or not all(prediction['checks'].values()):
        raise RuntimeError('short-step verdict differs')
    if prediction['choice']!=audit['choice'] or prediction['linf_upper']!=audit['linf_upper']:
        raise RuntimeError('reviewed coefficient changed')
    t=prediction['choice']['fraction']
    expected=.9*min(-prediction['rd']/prediction['dd'],prediction['linf_upper'],1.)
    if not 0<t<=prediction['linf_upper']<=1 or t!=expected:raise RuntimeError('fraction is not the registered constrained optimum')
    if any(prediction[k] for k in ('candidate_written','actual_map_performed','accepted_material_step')):
        raise RuntimeError('source must be a read-only prediction')
    return t
...
def prepare(out):
    ...
    claims=v.audited_inputs(local,ev/'20260926-heating-block-pilot-review.json',
        ev/'20260926-heating-block-pilot-77648-terminal.json',77648,names)
    review=pipeline.read(ev/'20260926-heating-block-pilot-review.json')
    ...
    sc=ROOT/SCAN
    scan_claims=v.audited_inputs(sc,ev/'20260926-short-step-review.json',
        ev/'20260926-block-short-step-v2-77783-terminal.json',77783,['declaration.json','prediction.json','status.json'])
    declared=pipeline.read(sc/'declaration.json');prediction=pipeline.read(sc/'prediction.json')
    audit=pipeline.read(ev/'20260926-short-step-review.json')
    if pipeline.read(sc/'status.json')['status']!='complete_requires_review':raise RuntimeError('scan not complete')
    fraction=reviewed_fraction(declared,prediction,audit,[inp,mapped])
    claims+=scan_claims+declared['claims']+declared['code']
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/validate_step21_block_short_step.sbatch',
        'tests/test_validate_step21_block_short_step.py','handoff/protocols/step21-block-short-step-validation-v1.md')]
    ...
    plan=dict(cases={'control':owned},claims=claims,code=code,selected='blocks24-and48-constrained-short-step',fraction=fraction,scan_prediction=prediction,source_scan=SCAN,
...
def execute(out):
    ...
    try:
        ...
        fields.write_candidates(paths[0],{int(i):ROOT/c['path'] for i,c in plan['replacements'].items()},
            seedpath,halfpath,pipeline.SHAPE,fraction=plan['fraction'],checkpoint=reused.checkpoint)
        ...
        with fixed.relay_dispatch():
            ...
            def validate():
                ...
                predicted=plan['scan_prediction']['choice']['predicted_l2_ratio']
                error=abs(comparison['fixed_scale_l2_ratios'][1]-predicted)/predicted
                ...
                result=dict(selected=plan['selected'],candidate=seed,half_candidate=halfseed,
                    ...
                    fraction=plan['fraction'],predicted_l2_relative_error=error,field_comparison=comparison,checks=checks,validated=all(checks.values()),
            ...
            def feedback(n):
                ...
                p['sources']['constrained_short_step_validation']=pipeline.claim(folder/'validation.json')
