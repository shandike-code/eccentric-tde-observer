"""Fresh baseline and two same-trial confirmations of the supported half-amplitude step19."""
import argparse
...
from operations.common_outer_step19 import verify_step19_pair
...
PRIOR='outputs/hpc/common-step19-20260924'
CONTROL='outputs/hpc/common-confirmation18-20260924/confirm2'
...
def confirmation_protocol(reference,folder,endpoints,rows,trial_claim,is_control):
    ...
    p['numerical_backtracking']={'alpha':0. if is_control else 1/128,
        'fixed_base_accepted_index':18,'fixed_direction':'r18','physical_time_advanced':False}
    return p


def validate_base_against_accepted(base,accepted):
    # 元数据可由第18步试探重置为第19步零控制，物理物质和encoded本身不可改。
    for k in ('encoded_state','temperature_k','hydrogen_fraction','helium_fraction','specific_material_energy_erg_g',
        ...
        if not np.array_equal(base[k],accepted[k]):raise RuntimeError('zero control not accepted x18: '+k)
...
def zero_feedback(folder,p,path,history):
    ...
    verify_step19_pair(folder,p,result)
    return all(gates.values())



def prepare(out):
    review_path=ROOT/'handoff/evidence/20260924-common-step19-review.json';review=pipeline.read(review_path)
    if review['accepted_outer_steps']!=18 or not review['pairs']['pair08']['all_16_pair_gates']:raise RuntimeError('Mac candidate audit missing')
    if pipeline.read(ROOT/PRIOR/'status.json')['status']!='candidate_supported_requires_review':raise RuntimeError('candidate not supported')
    terminal=pipeline.read(ROOT/'handoff/evidence/20260924-step19-76509-terminal.json')
    if terminal['job_id']!=76509 or terminal['state']!='COMPLETED':raise RuntimeError('source scheduler failure')
    ...
    claims=[pipeline.claim(review_path),pipeline.claim(ROOT/'handoff/evidence/20260924-step19-76509-terminal.json')]
    for relative in ('maps/trial_material.npz','maps/config.json','maps/state.json',
                     'inputs/outer_base_material.npz','inputs/base_residual.npy','inputs/feedback_reference.json',
                     'pair08/feedback_protocol.json','pair08/feedback_summary.json',
                     'pair08/previous_response.npz','pair08/final_response.npz','endpoints-map08/manifest.json'):
    ...
    summary=pipeline.read(ROOT/PRIOR/'pair08/feedback_summary.json')
    if not all_pair_gates(summary) or summary['comparison']!=review['pairs']['pair08']['comparison']:raise RuntimeError('source gate mismatch')
    if not review['pairs']['pair08']['fresh_baseline_comparison']['passed']:raise RuntimeError('fresh control comparison failed')
    observed=pipeline.read(ROOT/PRIOR/'pair08/feedback_protocol.json')
    reference_path=ROOT/PRIOR/'inputs/feedback_reference.json'
    ...
    old=load_arrays(ROOT/sources['physical_old_time_level']['path']);trial=load_arrays(ROOT/PRIOR/'maps/trial_material.npz')
    exact_trial(trial,base,r,old,1/128);validate_zero(base,base,r,old)
    accepted=load_arrays(ROOT/CONTROL/'trial_material.npz')
    # 仅零控制的迭代元数据改变；真正进入native的物理物质必须等于已接受x18。
    ...
    acceptance=pipeline.read(ROOT/'handoff/evidence/20260924-accepted-step18.json');reused.verify(acceptance['claims'])
    if acceptance['accepted_outer_steps']!=18:raise RuntimeError('step18 is not accepted')
    claims.append(pipeline.claim(ROOT/'handoff/evidence/20260924-accepted-step18.json'))
    state=pipeline.read(ROOT/CONTROL/'state.json');cfg=pipeline.read(ROOT/CONTROL/'config.json')
    if state['config_sha256']!=pipeline.sha256(ROOT/CONTROL/'config.json'):raise RuntimeError('base config mismatch')
    seeds={'control':latest_seed(state),'confirm1':pipeline.read(ROOT/PRIOR/'endpoints-map08/manifest.json')['endpoints']['mapped_final']}
    ...
    if not np.array_equal(r,load_arrays(ROOT/CONTROL/'common-feedback/final_response.npz')['residual']):raise RuntimeError('r18 changed')
    ...
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_confirmation_step19.sbatch',
        'tests/test_common_confirmation_step19.py','handoff/protocols/common-confirmation-step19-v1.md')]
    plan={'cases':{'base':{'trial':pipeline.claim(prepared/'trial_material.npz'),'config':pipeline.claim(prepared/'config.json')},
        'trial':{'trial':pipeline.claim(ROOT/PRIOR/'maps/trial_material.npz'),'config':pipeline.claim(ROOT/PRIOR/'maps/config.json')}},
        'code':code,'claims':claims,'seeds':seeds,'limits':LIMITS,'maximum_new_maps':8,'maximum_feedback_pairs':3,
        'accepted_outer_steps':18,'automatic_promotion':False,'physical_dt_changed':False,'environment':pipeline.environment()}
...
def execute(out):
    ...
    def mark(status,**kw):pipeline.write_json(out/'status.json',dict(status=status,updated_unix=time.time(),accepted_outer_steps=18,new_material_steps=0,**kw))
    mark('preparing')
    try:
        ...
        def evaluate(name,count):
            ...
            with reused.feedback_stop_guard():
                if name=='control':passed=zero_feedback(rd,p,path,rows)
                    ...
                    result=pair.run_pair(path,pipeline.sha256(path));verify_step19_pair(rd,p,result)
        ...
        reused.verify(claims+code);reused.immutable(out/'summary.json',{'status':status,'cases':reports,'accepted_outer_steps':18,'new_material_steps':0})
