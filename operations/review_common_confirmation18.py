...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,candidate_root,output):
    ...
    root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==17 and root['status']=='confirmed_requires_mac_review'
    ...
    for case in ('control','confirm1','confirm2'):
        ...
        assert p['numerical_backtracking']==dict(alpha=0. if case=='control' else 1/128,fixed_base_accepted_index=17,fixed_direction='r17',physical_time_advanced=False)
        ...
        if case=='control':
            ...
            frozen=arrays(candidate_root/'half/trial_material.npz');assert set(t)==set(frozen) and all(np.array_equal(t[k],frozen[k]) for k in t)
    ...
    # The new zero control retains accepted x17 exactly, not its response target.
    ...
    prior_vectors={e:arrays(candidate_root/'half/pair08'/(e+'_response.npz'))['residual'] for e in ('previous','final')}
    ...
    assert seeds['control']['sha256']==read(candidate_root/'control/state.json')['history'][-1]['output_sha256']
    assert seeds['confirm1']==read(candidate_root/'half/endpoints-map08/manifest.json')['endpoints']['mapped_final']
    for case in ('control','confirm1','confirm2'):
    ...
    record={'archive':read(receipt),'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_confirmation_drift':drift,'baseline_pair_drift':baseline_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':17,'cross_platform_decode_rtol':8*np.finfo(float).eps}
