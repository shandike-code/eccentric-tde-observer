"""Bounded thermal and population direction diagnostics at the accepted x20."""
import argparse
...
PRIOR='outputs/hpc/common-step21-probe-20260925'
...
LIMITS={'control':4,'thermal':8,'population':8}
...
def split_direction(r,name):
    if name not in ALPHAS:raise ValueError('undeclared direction')
    v=np.asarray(r)
    if v.ndim!=1 or v.size%4 or not np.all(np.isfinite(v)):raise ValueError('invalid encoded residual')
    # 只拆分试探位移；后续方程和四个残差分量仍完整求解，不删除任何层。
    d=np.array(v,copy=True).reshape(-1,4)
    if name=='thermal':d[:,1:]=0
    if name=='population':d[:,0]=0
    return d.reshape(-1)


def exact_trial(trial,base,r,old,name):
    alpha=ALPHAS[name];direction=split_direction(r,name)
    ...
    for k,v in [('base_encoded_state',base['encoded_state']),('base_residual',r),('finite_direction',direction),('encoded_state',base['encoded_state']+alpha*direction)]:
...
def make_trial(base,r,old,name):
    direction=split_direction(r,name);alpha=ALPHAS[name]
    t={k:np.array(v,copy=True) for k,v in base.items()}
    t.update(encoded_state=base['encoded_state']+alpha*direction,finite_direction=direction,relaxation=np.array(alpha))
    if alpha:
    ...
    exact_trial(t,base,r,old,name);return t


def direction_sequence(evaluate):
    if evaluate('control')!='baseline_stable':return 'stopped_at_control'
    outcomes={}
    for name in ('thermal','population'):
        outcomes[name]=evaluate(name)
        if outcomes[name]=='physical_domain_rejected':return 'stopped_at_'+name
    return 'direction_budget_complete_requires_review'
...
def validate_failure_audit(audit,terminal):
    if terminal['job_id']!=76871 or terminal['state']!='COMPLETED' or audit['accepted_outer_steps']!=20:raise RuntimeError('source audit missing')
    if set(audit['pairs'])!={'control','half04','half08'}:raise RuntimeError('incomplete audit')
    gates=audit['pairs']['half08']['gate_checks']
    ...
    if audit['pairs']['half08']['fresh_baseline_comparison']['passed']:raise RuntimeError('failure comparison changed')
...
def prepare(out):
    audit_path=ROOT/'handoff/evidence/20260925-common-step21-probe-review.json';audit=pipeline.read(audit_path)
    terminal_path=ROOT/'handoff/evidence/20260925-step21-probe-76871-terminal.json';terminal=pipeline.read(terminal_path)
    ...
    for name in ('control/trial_material.npz','control/config.json','control/state.json',
                 'half/trial_material.npz','half/config.json','half/state.json',
                 'half/pair04/feedback_summary.json','half/pair04/previous_response.npz','half/pair04/final_response.npz',
                 'half/pair08/feedback_summary.json','half/pair08/feedback_protocol.json','half/pair08/previous_response.npz','half/pair08/final_response.npz'):
    ...
    seed=latest_seed(pipeline.read(ROOT/PRIOR/'control/state.json'));claims+=[seed]+list(s.values())
    ...
    for name,alpha in ALPHAS.items():
        folder=inputs/name;folder.mkdir();t=make_trial(base,r,old,name);np.savez(folder/'trial_material.npz',**t)
        ...
        exact_trial(load_arrays(folder/'trial_material.npz'),base,r,old,name)
        claims+=list(cases[name].values())
    code=fresh.code_claims()+[pipeline.claim(ROOT/p) for p in ('operations/common_step21_directions.sbatch','tests/test_common_step21_directions.py','handoff/protocols/common-step21-directions-v1.md')]
    plan={'cases':cases,'seed':seed,'claims':claims,'code':code,'alphas':ALPHAS,'limits':LIMITS,'maximum_maps':20,'maximum_feedback_pairs':5,'diagnostic_only':True,'accepted_outer_steps':20,'automatic_promotion':False,'physical_dt_changed':False,'environment':pipeline.environment()}
...
def execute(out):
    ...
    mark('preparing')
    try:
        if shutil.disk_usage(out).free<28*pipeline.STATE_BYTES:raise RuntimeError('checkpoint disk budget insufficient')
            ...
            for _ in range(LIMITS[name]):
                ...
                if name=='control':
                    ...
                    if control_allows_probe(stable,report):
                        seeds['thermal']=latest_seed(state);seeds['population']=seeds['thermal']
                        return 'baseline_stable'
                    return 'baseline_unstable'
                # 两个方向均执行固定预算；任何原门通过也不能自动接受诊断状态。
            reports[name]=case_reports
            return 'diagnostic_budget_complete'
