...
def main() -> None:
    mapped = json.loads(
        (OUTPUT / "phase7b8e_backtracked_radiation_map_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        mapped["decision"]["phase7b8e_gate_passed"] is not True
        or mapped["decision"]["assembled_feedback_and_true_residual_diagnosis_authorized"]
        is not True
    ):
        raise RuntimeError("Phase 7B8f requires passed Phase 7B8e")
    payload = {
        "phase": "7B8f backtracked assembled feedback and triple true-residual gate",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] assembled formal-source and three residual contraction "
            "gates after feedback-informed backtracking; [V] global H/He rates, "
            "frame consistency, symmetry and resources; [O] no further update"
        ),
        "sources": {
            "phase7b8e_summary": _source(
                "outputs/phase7b8e_backtracked_radiation_map_summary.json"
            ),
            "phase7b8e_protocol": _source(
                "outputs/phase7b8e_preregistered_backtracked_radiation_map.json"
            ),
            "phase7b8e_runner": _source(
                "scripts/phase7b8e_backtracked_radiation_map.py"
            ),
            "mapped_radiation_state": _source(
                "outputs/checkpoints/phase7b8e_backtracked_radiation_map.dat"
            ),
            ...
            "phase7b8c_runner": _source("scripts/phase7b8c_secant_feedback.py"),
        ...
        "configuration": {
            ...
            "halo_state": "globally assembled Phase 7B8e mapped state",
            "fixed_point_residual_time_base": "unchanged physical old material time level",
        ...
        "gates": {
            ...
            "all_assembled_arrays_finite": True,
            "all_atomic_rates_nonnegative": True,
            ...
            "mass_weighted_true_residual_contraction_below": 0.5,
            "previous_limiting_cell_true_residual_contraction_below": 0.8,
            "maximum_cell_true_residual_contraction_below": 0.995,
        ...
        "authorization": {
            "accept_backtracking_strategy_only_if_all_three_true_residual_gates_pass": True,
            "accept_as_coupled_fixed_point_only_if_fixed_point_acceptance_passes": True,
            "another_material_update": False,
            "full_orbit": False,
            "phase4_replacement": False,
        },
    }
    path = OUTPUT / "phase7b8f_preregistered_backtracked_feedback.json"
