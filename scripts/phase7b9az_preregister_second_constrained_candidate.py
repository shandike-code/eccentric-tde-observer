"""Phase 7B9az：冻结第二轮正性约束 Anderson 候选审计。"""
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9az requires the constrained second basis")
    for label in ("x2", "x3", "x4"):
        if helper._sha256(ROOT / source[f"{label}_state_path"]) != source[
            f"{label}_state_sha256"
        ]:
            raise RuntimeError(f"Phase 7B9az {label} state changed")
    ...
    payload = {
        "phase": "7B9az second constrained Anderson candidate audit and write",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] audit the frozen cutting-plane coefficient against "
            "the original maximum-norm operator, mapped-state positivity and boundary "
            "functionals, with x4 as the control; [V] one fresh F(x4), recoverable "
            "candidate write only after at least 20 percent improvement; [O] a written "
            "candidate still requires an independent global self audit"
        ),
        "sources": {
            "phase7b9ay_summary": helper._source(
                "outputs/phase7b9ay_fused_constrained_anderson_summary.json"
            ),
            "phase7b9ay_protocol": helper._source(
                "outputs/phase7b9ay_preregistered_fused_constrained_anderson.json"
            ),
        ...
        "configuration": {
            ...
            "audit_report_directory": (
                "outputs/checkpoints/phase7b9az_second_candidate/audit_reports"
            ),
            "manifest_path": (
                "outputs/checkpoints/phase7b9az_second_candidate/manifest.json"
            ),
            "summary_path": (
                "outputs/phase7b9az_second_constrained_candidate_summary.json"
            ),
            "figure_path": (
                "outputs/phase7b9az_second_constrained_candidate.png"
            ),
            "runner_path": (
                "scripts/phase7b9az_second_constrained_candidate.py"
            ),
    ...
    path = OUTPUT / "phase7b9az_preregistered_second_constrained_candidate.json"
