"""Phase 7B9dh：仅供 0.0625 fixed-material continuation 使用的独立序列核。"""
...
OUTPUT = ROOT / "outputs"
# 由 hash-pinned continuation runner 在父进程和每个 worker 中显式注入。
EXPECTED_PROTOCOL_SHA256 = "UNSET_HALF_TRIAL_CONTINUATION_PROTOCOL_SHA256"
base = generic.base
...
def _feedback_authorization(
    configuration: dict[str, object], *, converged: bool
) -> tuple[bool, bool]:
    """Keep legacy authorization while allowing a provisional-only fixed-matter pause."""
    mode = configuration.get(
        "material_feedback_authorization_mode", "after_first_low_residual"
    )
    if mode not in {
        "after_first_low_residual",
        "after_two_consecutive_and_formal_pair",
    }:
        raise RuntimeError("Phase 7B9al material-feedback authorization mode changed")
    return (
        bool(converged and mode == "after_first_low_residual"),
        bool(converged and mode == "after_two_consecutive_and_formal_pair"),
    )


def _initial_reproduction_gate(
    ...
    """Return applicability and pass flag for the optional first-map reproduction."""
    mode = configuration.get(
        "initial_reference_mode", "require_frozen_residual_reproduction"
    )
    if mode not in {
        "require_frozen_residual_reproduction",
        "unreferenced_fixed_material_bootstrap",
    }:
        raise RuntimeError("Phase 7B9al initial-reference mode changed")
    # 中文：新物质态没有可复现的旧残差；首图只作 bootstrap，仍完整计算物理残差。
    applicable = not (
        iteration == 0 and mode == "unreferenced_fixed_material_bootstrap"
    )
    if not applicable or iteration != 0:
        return applicable, True
    tolerance = float(gates["initial_audit_absolute_tolerance"])
    passed = bool(
        abs(
...
def _summary(
    ...
    feedback_authorized, provisional_authorized = _feedback_authorization(
        configuration, converged=converged
    )
    payload = {
        ...
        "decision": {
            ...
            "material_feedback_authorized": feedback_authorized,
            "provisional_feedback_extraction_authorized": provisional_authorized,
...
def run(
    ...
    while len(manifest["iterations"]) < int(configuration["maximum_picard_maps"]):
        ...
        (
            initial_reproduction_applicable,
            initial_reproduction,
        ) = _initial_reproduction_gate(
            configuration,
            reference,
            gates,
            metrics,
        ...
        record = {
            ...
            **metrics,
            ...
            "initial_reproduction_applicable": initial_reproduction_applicable,
            "gate_checks": checks,
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=OUTPUT / "phase7b9dh_preregistered_half_trial_radiation.json",
    )
