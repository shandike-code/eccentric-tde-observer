"""Phase 7B9o：冻结静态同频对角 ALI 代表块准入门。"""
...
def main() -> None:
    ...
    prior = json.loads(
        (OUTPUT / "phase7b9n_multimode_krylov_pilot_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        ...
        or prior["decision"]["multimode_minimum_residual_pilot_passed"] is not False
        or prior["decision"]["one_full_frequency_multimode_map_authorized"]
        is not False
    ):
        raise RuntimeError("Phase 7B9o requires the completed 7B9n failure state")
    current = ROOT / manifest["current_state_path"]
    if _sha256(current) != manifest["current_state_sha256"]:
        raise RuntimeError("Phase 7B9o current map-6 state changed")
    worker_sources = {
    ...
    payload = {
        "phase": "7B9o static same-frequency diagonal ALI pilot",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] exact step-characteristic local Lambda diagonal, "
            "static same-frequency rank-one scattering inverse and one exact-positive "
            "correction; [V] fresh mixed-frame original-operator residual; "
            "[O] Doppler frequency coupling remains outside the preconditioner"
        ),
        "sources": {
            "phase7b9n_summary": _source(
                "outputs/phase7b9n_multimode_krylov_pilot_summary.json"
            ),
        ...
        "configuration": {
            ...
            "scattering_inverse": (
                "static same-lab-frequency angular rank-one inverse at each depth"
            ),
            "doppler_frequency_coupling_inside_preconditioner": False,
        ...
        "gates": {
            ...
            "each_minimum_scattering_denominator_strictly_above": 1.0e-8,
        ...
        "authorization": {
            "one_full_frequency_static_diagonal_ali_map_if_gate_passes": True,
            "higher_history_or_scalar_extrapolation": False,
    ...
    path = OUTPUT / "phase7b9o_preregistered_static_diagonal_ali_pilot.json"
