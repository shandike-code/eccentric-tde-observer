"""Phase 7B9bf：冻结当前受约束候选的仿射 Krylov 映射。"""
...
OUTPUT = ROOT / "outputs"
SCRATCH = "outputs/checkpoints/phase7b6f_full_frequency_iteration4.dat"
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9bf requires the audited unconverged candidate")
    ...
    if helper._sha256(input_state) != audit["input_state_sha256"]:
        raise RuntimeError("Phase 7B9bf input state changed")
    if output_state.stat().st_size != input_state.stat().st_size:
        raise RuntimeError("Phase 7B9bf scratch checkpoint has the wrong size")
    payload = {
        "phase": "7B9bf audited candidate affine Krylov map",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] apply one unrelaxed fixed-matter source map to the "
            "independently audited candidate and reuse one explicitly superseded "
            "generated checkpoint as recoverable scratch; [V] ownership, positivity, "
            "residual reproduction, boundary and resources; [O] no matter feedback"
        ),
        "sources": {
        ...
        "configuration": {
            ...
            "manifest_path": (
                "outputs/checkpoints/phase7b9bf_candidate_affine_map/manifest.json"
            ),
            "report_directory": (
                "outputs/checkpoints/phase7b9bf_candidate_affine_map/reports"
            ),
            "summary_path": "outputs/phase7b9bf_candidate_affine_map_summary.json",
            "figure_path": "outputs/phase7b9bf_candidate_affine_map.png",
            "block_report_prefix": "phase7b9bf",
            "runner_path": "scripts/phase7b9bf_candidate_affine_map.py",
            "input_audit_source_key": "phase7b9be_summary",
            ...
            "scratch_reuse_note": (
                "Phase 7B6f iteration-4 raw checkpoint is superseded by later audited "
    ...
    path = OUTPUT / "phase7b9bf_preregistered_candidate_affine_map.json"
