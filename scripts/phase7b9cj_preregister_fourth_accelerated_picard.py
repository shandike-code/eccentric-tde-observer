"""Phase 7B9cj：冻结第三次 Anderson 映射后的两步 Picard 延拓。"""
...
def main() -> None:
    ...
    anchor = json.loads(
        (OUTPUT / "phase7b9ci_third_map_anchor_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        mapped["decision"]["global_positive_picard_map_passed"] is not True
        or anchor["decision"]["fourth_accelerated_picard_continuation_authorized"]
        is not True
        or anchor["anchor_state_sha256"] != mapped["output_state_sha256"]
    ):
        raise RuntimeError("Phase 7B9cj requires the audited third map and anchor")
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9cj checkpoint chain changed")
    scratch_sha = helper._sha256(scratch)
    payload = {
        "phase": "7B9cj fourth accelerated fixed-matter positive Picard continuation",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] import the independently audited Phase 7B9ch map, "
        ...
        "sources": {
            ...
            "phase7b9ci_summary": helper._source(
                "outputs/phase7b9ci_third_map_anchor_summary.json"
            ),
            "phase7b9ci_protocol": helper._source(
                "outputs/phase7b9ci_preregistered_third_map_anchor.json"
            ),
        ...
        "configuration": {
            ...
            "seed_summary_source_key": "phase7b9ch_summary",
            ...
            "manifest_path": (
                "outputs/checkpoints/phase7b9cj_fourth_accelerated_picard/manifest.json"
            ),
            "report_directory": (
                "outputs/checkpoints/phase7b9cj_fourth_accelerated_picard/reports"
            ),
            "summary_path": "outputs/phase7b9cj_fourth_accelerated_picard_summary.json",
            "figure_path": "outputs/phase7b9cj_fourth_accelerated_picard.png",
            "runner_path": "scripts/phase7b9cj_fourth_accelerated_picard.py",
        ...
        "authorization": {
            ...
            "fourth_slow_mode_test_after_pause": True,
    ...
    path = OUTPUT / "phase7b9cj_preregistered_fourth_accelerated_picard.json"
