"""Phase 7B9au：冻结正性约束 Anderson 写后态的独立审计。"""
...
def main() -> None:
    written = json.loads(
        (
            OUTPUT / "phase7b9at_constrained_anderson_candidate_summary.json"
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9au requires the passed written candidate")
    state_relative = str(written["candidate_state_path"])
    if _sha256(ROOT / state_relative) != written["candidate_state_sha256"]:
        raise RuntimeError("Phase 7B9at written state changed")
    payload = {
        "phase": "7B9au constrained Anderson candidate global self audit",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] apply one fresh original source map to the written "
            "positivity-constrained Anderson state; [V] reproduce its predicted "
        ...
        "configuration": {
            ...
            "report_directory": (
                "outputs/checkpoints/phase7b9au_constrained_anderson_audit/reports"
            ),
            "summary_path": (
                "outputs/phase7b9au_constrained_anderson_audit_summary.json"
            ),
            "figure_path": (
                "outputs/phase7b9au_constrained_anderson_audit.png"
            ),
            "block_report_prefix": "phase7b9au",
            "runner_path": (
                "scripts/phase7b9au_constrained_anderson_audit.py"
            ),
            "prior_map_summary_source_key": "phase7b9at_summary",
    ...
    path = OUTPUT / "phase7b9au_preregistered_constrained_anderson_audit.json"
