"""Phase 7B9ch：冻结第三次慢模 Anderson 候选的完整原算子映射。"""
...
def main() -> None:
    candidate = json.loads(
        (OUTPUT / "phase7b9cg_third_slow_mode_anderson_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        ...
        raise RuntimeError("Phase 7B9ch requires the passed third slow-mode candidate")
    ...
    if helper._sha256(input_state) != candidate["candidate_state_sha256"]:
        raise RuntimeError("Phase 7B9ch candidate checkpoint changed")
    if output_state.stat().st_size != input_state.stat().st_size:
        raise RuntimeError("Phase 7B9ch mapped scratch size is invalid")
    payload = {
        "phase": "7B9ch third slow-mode candidate original-operator map",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] apply exactly one complete, unrelaxed fixed-matter "
            "source map to the third protected Anderson(1) state; [V] audit all "
            "9632 groups, positivity, prediction reproduction, boundary and "
            "resources; [O] material feedback remains closed above 1e-4"
        ),
        "sources": {
            "phase7b9cg_summary": helper._source(
                "outputs/phase7b9cg_third_slow_mode_anderson_summary.json"
            ),
            "phase7b9cg_protocol": helper._source(
                "outputs/phase7b9cg_preregistered_third_slow_mode_anderson.json"
            ),
        ...
        "configuration": {
            ...
            "manifest_path": (
                "outputs/checkpoints/phase7b9ch_third_candidate_map/manifest.json"
            ),
            "report_directory": (
                "outputs/checkpoints/phase7b9ch_third_candidate_map/reports"
            ),
            "summary_path": "outputs/phase7b9ch_third_candidate_map_summary.json",
            "figure_path": "outputs/phase7b9ch_third_candidate_map.png",
            "block_report_prefix": "phase7b9ch",
            "runner_path": "scripts/phase7b9ch_third_candidate_map.py",
            "input_audit_source_key": "phase7b9cg_summary",
    ...
    path = OUTPUT / "phase7b9ch_preregistered_third_candidate_map.json"
