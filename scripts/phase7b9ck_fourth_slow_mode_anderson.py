"""Phase 7B9ck：执行第四次受保护 Anderson(1) 慢模外推。"""

from __future__ import annotations

import argparse
from pathlib import Path

try:
    from scripts import phase7b9bx_slow_mode_anderson as generic
except ModuleNotFoundError as error:
    if error.name != "scripts":
        raise
    import phase7b9bx_slow_mode_anderson as generic  # type: ignore[no-redef]


ROOT = Path(__file__).resolve().parents[1]
EXPECTED_PROTOCOL_SHA256 = "7f2fd25866e2a6f6bd3b96b07e71fb5572d0d17097b6909191a2661fb1f78f52"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--protocol",
        type=Path,
        default=(
            ROOT / "outputs/phase7b9ck_preregistered_fourth_slow_mode_anderson.json"
        ),
    )
    args = parser.parse_args()
    generic.EXPECTED_PROTOCOL_SHA256 = EXPECTED_PROTOCOL_SHA256
    generic.run(args.protocol)


if __name__ == "__main__":
    main()
