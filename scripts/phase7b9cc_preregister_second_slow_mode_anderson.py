"""Phase 7B9cc：冻结加速轨迹上的第二次 Anderson(1) 慢模外推。"""
...
def main() -> None:
    sequence = json.loads(
        (OUTPUT / "phase7b9cb_second_slow_mode_summary.json").read_text(
            encoding="utf-8"
        )
    )
    anchor = json.loads(
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9cc requires three audited accelerated records")
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9cc states are not consecutive")
    for label, row in basis.items():
        if helper._sha256(ROOT / row["path"]) != row["sha256"]:
            raise RuntimeError(f"Phase 7B9cc {label} checkpoint changed")
    candidate = ROOT / CANDIDATE_OUTPUT
    expected_size = (ROOT / basis["x9"]["path"]).stat().st_size
    if candidate.stat().st_size != expected_size:
        raise RuntimeError("Phase 7B9cc candidate scratch size is invalid")
    payload = {
        "phase": "7B9cc second protected Anderson(1) slow-mode candidate",
        "protocol_version": 1,
        "classification": (
            "[A-informed] two new consecutive residual vectors follow the first "
            "audited slow-mode jump; [A-preregistered] repeat the same global L2 "
            "line minimum, exact positivity and 96-map horizon guards without "
            "retuning thresholds; [V] require maximum-norm and boundary improvement; "
            "[O] a fresh complete original-operator map remains mandatory"
        ),
        "sources": {
            "phase7b9cb_summary": helper._source(
                "outputs/phase7b9cb_second_slow_mode_summary.json"
            ),
            "phase7b9cb_protocol": helper._source(
                "outputs/phase7b9cb_preregistered_second_slow_mode.json"
            ),
        ...
        "configuration": {
            ...
            "summary_path": "outputs/phase7b9cc_second_slow_mode_anderson_summary.json",
            "figure_path": "outputs/phase7b9cc_second_slow_mode_anderson.png",
    ...
    path = OUTPUT / "phase7b9cc_preregistered_second_slow_mode_anderson.json"
