"""Phase 7B5e：强度、消光与发射 Lorentz 变换的分量级控制。"""
...
COMPONENT_CONTROLS = (
    ...
    ("no intensity Lorentz", {"apply_intensity_lorentz": False}),
    ("no extinction Lorentz", {"apply_extinction_lorentz": False}),
    ("no emissivity Lorentz", {"apply_emissivity_lorentz": False}),
)
...
def _plot_component_response(path: Path, rows) -> None:
    cases = (
        "cold coefficient surface",
        "maximum cell speed",
        "maximum width change",
    )
    controls = [name for name, _ in COMPONENT_CONTROLS]
    ...
    for index, case in enumerate(cases):
        ...
        for bars in bar_sets:
            for bar, row in zip(bars, selected, strict=True):
                if not row["controlled_equation_passed"]:
                    bar.set_hatch("///")
                    bar.set_alpha(0.45)
    for axis in axes.flat:
        axis.set_xticks(x, controls, rotation=25, ha="right")
        axis.grid(alpha=0.25, axis="y")
    axes[0, 0].set_yscale("log")
    axes[0, 0].set_ylabel("Absolute H I rate error")
    axes[0, 0].set_title("Matched component-control errors")
    ...
    axes[0, 1].set_title("Signed Lorentz-component response")
    ...
    axes[1, 0].set_title("Component-removal response")
    axes[1, 1].axhline(
        COUPLED_RESIDUAL_TARGET, color="black", linestyle=":"
    )
    axes[1, 1].set_yscale("log")
    axes[1, 1].set_ylabel("Candidate coupled residual")
    axes[1, 1].set_title("Controlled equation residual (hatched = failed gate)")
        ...
        for row_index in range(matrix.shape[0]):
            for column_index in range(matrix.shape[1]):
                axis.text(
                    ...
                    color=(
                        "white"
                        if matrix[row_index, column_index] < 0.35
                        else "black"
                    ),
                    fontsize=8,
                )
    fig.colorbar(
        image, ax=axes, label="Fraction of absolute H I integrand difference"
    )
    fig.savefig(path, dpi=180)
    plt.close(fig)
...
def run_all(output_dir: Path, material_path: Path, *, force: bool) -> None:
    paths = {
        "summary": output_dir / "phase7b5e_summary.json",
        "controls": output_dir / "phase7b5e_lorentz_component_controls.csv",
        "regions": output_dir / "phase7b5e_region_response.csv",
        "component_plot": output_dir / "phase7b5e_lorentz_component_controls.png",
        "region_plot": output_dir / "phase7b5e_threshold_region_response.png",
    }
    ...
    for definition in definitions:
        ...
        for control_name, control_kwargs in COMPONENT_CONTROLS:
            ...
            rows.append(
                {
                    ...
                    "candidate_final_fixed_point_change": candidate_row[
                        "final_fixed_point_change"
                    ],
                    "reference_fixed_point_iterations": reference_row[
                        "fixed_point_iterations"
                    ],
                    "reference_final_fixed_point_change": reference_row[
                        "final_fixed_point_change"
                    ],
    ...
    for case in {row["case"] for row in rows}:
        full_error = next(
            ...
            for row in rows
            if row["case"] == case and row["control"] == "full Lorentz operator"
        )
        for row in rows:
            if row["case"] == case:
    ...
    previous = json.loads(
        (output_dir / "phase7b5d_summary.json").read_text(encoding="utf-8")
    )
    previous_by_case = {
        row["case"]: row
        for row in previous["operator_controls"]
        if row["control"] == "full operator"
    }
    baseline_reproduction = max(
        _relative_difference(
            row["absolute_relative_error"],
            previous_by_case[row["case"]]["absolute_relative_error"],
        )
        for row in rows
    ...
    for row in rows:
        row["controlled_equation_passed"] = bool(
            row["candidate_rate_reconciliation_relative"]
            < RATE_RECONCILIATION_TARGET
            and row["reference_rate_reconciliation_relative"]
            < RATE_RECONCILIATION_TARGET
    ...
    admissible_controls = [
        control_name
        for control_name, _ in COMPONENT_CONTROLS
        if all(
            row["controlled_equation_passed"]
            for row in rows
            if row["control"] == control_name
        )
    ]
    moving_cases = ("maximum cell speed", "maximum width change")
    stable_controls = []
    for control_name, _ in COMPONENT_CONTROLS[1:]:
        selected = [
            row
            for row in rows
            if row["control"] == control_name and row["case"] in moving_cases
        ]
        if control_name in admissible_controls and all(
            row["error_reduction_relative_to_full"]
            >= STABLE_MOVING_ERROR_REDUCTION
            for row in selected
        ):
            stable_controls.append(control_name)
    all_component_controls_admissible = all(
        name in admissible_controls for name, _ in COMPONENT_CONTROLS[1:]
    )
    unique_stable = (
        all_component_controls_admissible and len(stable_controls) == 1
    )
    intensity_branch_response = "no intensity Lorentz" in stable_controls
    valid_baseline = baseline_reproduction < BASELINE_REPRODUCTION_TARGET
    decision = {
        "baseline_reproduction_passed": valid_baseline,
        "all_controlled_equations_passed": all_control_operators_passed,
        "admissible_controls": admissible_controls,
        "stable_moving_error_reduction_controls": stable_controls,
        "unique_stable_lorentz_component_identified": unique_stable,
        "intensity_branch_single_pass_audit_authorized": bool(
            valid_baseline and intensity_branch_response
        ),
        "pairwise_lorentz_interaction_gate_authorized": False,
        "targeted_component_analytic_audit_authorized": False,
    ...
    _plot_component_response(paths["component_plot"], rows)
    _plot_region_response(paths["region_plot"], region_rows)
    report = {
        "phase": "7B5e",
        "classification": (
            "[A] partial unphysical Lorentz-component removals and 50% moving-state "
            "reduction criterion; [V] matched log-P1/P0 recomputation, controlled "
            "equation residuals and H I threshold localization; [O] branch-level "
            "analytic defect test and any production remediation"
        ),
        "configuration": {
        ...
        "control_semantics": {
            "no intensity Lorentz": (
                "set the lab-to-comoving intensity Doppler factor and angular "
                "aberration to identity, including the matched outer Planck state"
            ),
            "no extinction Lorentz": (
                "set only the comoving-to-lab extinction Doppler factor to identity"
            ),
            "no emissivity Lorentz": (
                "set only the comoving-to-lab emissivity Doppler factor to identity"
            ),
            "warning": (
                "partial controls are non-covariant localization experiments; "
                "their effects are nonlinear and do not authorize a physical fix"
            ),
        },
        "baseline_reproduction_relative": baseline_reproduction,
        "component_controls": rows,
        "region_response": region_rows,
        "decision": decision,
        "open_items": [
            "a removal response identifies sensitivity, not an implementation defect",
            "the extinction-only and emissivity-only controls fail the log-P1 residual gate",
            "the valid intensity response authorizes a single-pass transform audit only",
            "no unique Lorentz component has been identified while two controls are inadmissible",
            "the 38496-group P0 result remains a finite controlled reference",
            "no partial Lorentz control is a physical production spectrum",
            "angle, radiation-subgrid, orbit and matter-feedback gates remain closed",
        ],
        "figures": {
            "lorentz_component_controls": paths["component_plot"].name,
