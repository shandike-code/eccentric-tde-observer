"""Phase 7B9by：冻结慢模 Anderson 候选的一次完整原算子映射。"""
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("Phase 7B9by requires the passed Phase 7B9bx candidate")
    ...
    if helper._sha256(input_state) != candidate["candidate_state_sha256"]:
        raise RuntimeError("Phase 7B9by candidate checkpoint changed")
    if output_state.stat().st_size != input_state.stat().st_size:
        raise RuntimeError("Phase 7B9by mapped scratch size is invalid")
    payload = {
        "phase": "7B9by protected slow-mode candidate original-operator map",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] apply exactly one unrelaxed complete fixed-matter "
            "source map to the protected Anderson(1) state; [V] audit all 9632 "
            "frequency groups, positivity, algebraic-prediction reproduction, "
            "boundary flux and resources; [O] a residual below 1e-4 and a second "
            "consecutive state remain mandatory before material feedback"
        ),
        "sources": {
            "phase7b9bx_summary": helper._source(
                "outputs/phase7b9bx_slow_mode_anderson_summary.json"
            ),
            "phase7b9bx_protocol": helper._source(
                "outputs/phase7b9bx_preregistered_slow_mode_anderson.json"
            ),
        ...
        "configuration": {
            ...
            "manifest_path": (
                "outputs/checkpoints/phase7b9by_slow_mode_candidate_map/manifest.json"
            ),
            "report_directory": (
                "outputs/checkpoints/phase7b9by_slow_mode_candidate_map/reports"
            ),
            "summary_path": "outputs/phase7b9by_slow_mode_candidate_map_summary.json",
            "figure_path": "outputs/phase7b9by_slow_mode_candidate_map.png",
            "block_report_prefix": "phase7b9by",
            "runner_path": "scripts/phase7b9by_slow_mode_candidate_map.py",
            "input_audit_source_key": "phase7b9bx_summary",
            ...
            "scratch_reuse_note": (
                "Only the named superseded Phase 7B6j residual-14 generated "
                "checkpoint may be overwritten"
            ),
        ...
        "authorization": {
            ...
            "continue_fixed_matter_iteration_if_above_target": True,
            "second_consecutive_original_residual_required_if_below_target": True,
            "material_feedback": False,
            "accept_dynamic_nlte_solution": False,
        },
    }
    path = OUTPUT / "phase7b9by_preregistered_slow_mode_candidate_map.json"
