"""Phase 7B9ce：冻结第二次 Anderson 映射态的锚点副本。"""
...
def main() -> None:
    ...
    if mapped["decision"]["global_positive_picard_map_passed"] is not True:
        raise RuntimeError("Phase 7B9ce requires the audited second candidate map")
    source_relative = str(mapped["output_state_path"])
    source = ROOT / source_relative
    target = ROOT / TARGET
    if helper._sha256(source) != mapped["output_state_sha256"]:
        raise RuntimeError("Phase 7B9ce mapped source changed")
    if source.stat().st_size != target.stat().st_size:
        raise RuntimeError("Phase 7B9ce anchor size changed")
    payload = {
        "phase": "7B9ce preserved second accelerated-map anchor",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] preserve the complete second Anderson mapped state "
        ...
        "configuration": {
            ...
            "summary_path": "outputs/phase7b9ce_second_map_anchor_summary.json",
            "postcopy_authorization_key": (
                "third_accelerated_picard_continuation_authorized"
            ),
    ...
    path = OUTPUT / "phase7b9ce_preregistered_second_map_anchor.json"
