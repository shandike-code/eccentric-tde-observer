"""Phase 7B9af：冻结第二次 Picard 后的收缩审计。"""
...
def main() -> None:
    mapped = json.loads(
        (OUTPUT / "phase7b9ae_second_positive_picard_map_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        mapped["decision"]["global_positive_picard_map_passed"] is not True
        or mapped["decision"]["mapped_state_self_guard_residual_audit_authorized"]
        is not True
    ):
        raise RuntimeError("Phase 7B9af requires the passed Phase 7B9ae map")
    state_relative = str(mapped["output_state_path"])
    state = ROOT / state_relative
    if _sha256(state) != mapped["output_state_sha256"]:
        raise RuntimeError("Phase 7B9ae mapped state changed")
    payload = {
        "phase": "7B9af second positive Picard self-guard contraction audit",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] evaluate the second positive Picard state with one "
            "fresh original source map and self-consistent frequency guards; [V] "
            "global residual, contraction, boundary, positivity and resource audit; "
            "[O] no further map and no material feedback in this stage"
        ),
        "sources": {
            "phase7b9ae_summary": _source(
                "outputs/phase7b9ae_second_positive_picard_map_summary.json"
            ),
            "phase7b9ae_protocol": _source(
                "outputs/phase7b9ae_preregistered_second_positive_picard_map.json"
            ),
        ...
        "configuration": {
            ...
            "input_state_sha256": mapped["output_state_sha256"],
            "report_directory": (
                "outputs/checkpoints/phase7b9af_second_contraction/reports"
            ),
            "summary_path": (
                "outputs/phase7b9af_second_picard_contraction_audit_summary.json"
            ),
            "figure_path": (
                "outputs/phase7b9af_second_picard_contraction_audit.png"
            ),
            "block_report_prefix": "phase7b9af",
            "runner_path": "scripts/phase7b9af_second_picard_contraction_audit.py",
            "prior_map_summary_source_key": "phase7b9ae_summary",
        ...
        "authorization": {
            "continue_positive_picard_only_if_contraction_gate_passes": True,
    ...
    path = OUTPUT / "phase7b9af_preregistered_second_picard_contraction_audit.json"
