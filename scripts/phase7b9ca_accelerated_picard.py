"""Phase 7B9ca：从已审计慢模映射继续可恢复正 Picard 序列。"""
...
OUTPUT = ROOT / "outputs"
EXPECTED_PROTOCOL_SHA256 = "64a895b289d926172c9aa95b0955b2d612ca44c9ec58ec6a63a4b511f16c1bff"
...
def _seed_manifest(protocol_path: Path) -> None:
    if engine.base._sha256(protocol_path) != EXPECTED_PROTOCOL_SHA256:
        raise RuntimeError("frozen Phase 7B9ca protocol changed")
    ...
    if manifest_path.exists():
        return
    mapped = json.loads(
        (ROOT / protocol["sources"]["phase7b9by_summary"]["path"]).read_text(
            encoding="utf-8"
        )
    )
    anchor = ROOT / cfg["immutable_anchor_path"]
    initial = ROOT / cfg["initial_state_path"]
    scratch = ROOT / cfg["scratch_state_path"]
    if (
        engine.base._sha256(anchor) != cfg["immutable_anchor_sha256"]
        or engine.base._sha256(initial) != cfg["initial_state_sha256"]
        or engine.base._sha256(scratch) != cfg["scratch_state_initial_sha256"]
    ):
        raise RuntimeError("Phase 7B9ca initial checkpoint changed")
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=OUTPUT / "phase7b9ca_preregistered_accelerated_picard.json",
    )
