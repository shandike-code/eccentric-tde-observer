"""Phase 7B9be：执行第三轮 Anderson 写后态独立全局审计。"""
...
OUTPUT = ROOT / "outputs"
EXPECTED_PROTOCOL_SHA256 = "6a7c75984f9cde491738dcd381b4166aa4bcce8446ff78bfa2552d04dac2be86"
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=OUTPUT / "phase7b9be_preregistered_third_candidate_audit.json",
    )
    ...
    written = json.loads(
        (ROOT / protocol["sources"]["phase7b9bd_summary"]["path"]).read_text(
            encoding="utf-8"
        )
    )
