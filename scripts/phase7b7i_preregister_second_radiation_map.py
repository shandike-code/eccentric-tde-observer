"""Phase 7B7i：冻结第二物质迭代态上的一次全频辐射映射。"""
...
def main() -> None:
    material = json.loads(
        (OUTPUT / "phase7b7h_second_picard_direction_summary.json").read_text(
            encoding="utf-8"
        )
    )
    if material["decision"]["phase7b7h_gate_passed"] is not True:
        raise RuntimeError("Phase 7B7i requires passed Phase 7B7h")
    if (
        material["decision"][
            "one_full_frequency_radiation_directional_map_authorized"
        ]
        is not True
    ):
        raise RuntimeError("Phase 7B7i lacks radiation-map authorization")
    payload = {
        "phase": "7B7i second full-frequency radiation directional map",
        "protocol_version": 1,
        "classification": (
            "[A-preregistered] one block-Jacobi radiation source map at the second "
            "fixed-time-level material iterate; [V] exact frequency ownership, "
            "positivity and bounded resources; [O] assembled source/rate diagnostics "
            "are deliberately deferred to Phase 7B7j"
        ),
        "sources": {
            "phase7b7h_summary": _source(
                "outputs/phase7b7h_second_picard_direction_summary.json"
            ),
            "phase7b7h_protocol": _source(
                "outputs/phase7b7h_preregistered_second_picard_direction.json"
            ),
        ...
        "configuration": {
            ...
            "initial_radiation_state": "globally assembled Phase 7B7e mapped state",
            "initial_ALE_storage_term": (
                "unchanged original physical-time-level material Planck state"
            ),
            "collision_coefficients": "Phase 7B7h second material iterate",
        ...
        "authorization": {
            "assembled_source_and_atomic_rate_diagnostics_if_passes": True,
    ...
    path = OUTPUT / "phase7b7i_preregistered_second_radiation_map.json"
