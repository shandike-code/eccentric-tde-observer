"""Phase 7B9cz：用 7B9cy 审计模板冻结正式 H/He feedback pair。"""
...
PROTOCOL_PATH = ROOT / "outputs/phase7b9cz_preregistered_formal_feedback_pair.json"
...
def main() -> None:
    payload = adapter.build_formal_feedback_pair_protocol(
        ...
        adapter.FormalFeedbackPairProtocolSpec(
            phase="7B9cz formal H/He feedback pair after dependency migration audit",
            phase_index=1427,
            classification=(
                "[A-preregistered]+[V-code-path]+[V]+[O]：在 7B9cy 正路径逐数组"
                "一致性审计后，比较连续收敛 X/Y 辐射态的正式 H/He 反馈。"
            ),
            ...
            feedback_work_directory=(
                "outputs/checkpoints/phase7b9cz_formal_feedback_pair"
            ),
            previous_feedback_output="outputs/phase7b9cz_previous_feedback.npz",
            final_feedback_output="outputs/phase7b9cz_final_feedback.npz",
            target_material_output="outputs/phase7b9cz_target_material_state.npz",
            encoded_residual_output="outputs/phase7b9cz_encoded_residual.npy",
            summary_path="outputs/phase7b9cz_formal_feedback_pair_summary.json",
            figure_path="outputs/phase7b9cz_formal_feedback_pair.png",
        ),
    )
