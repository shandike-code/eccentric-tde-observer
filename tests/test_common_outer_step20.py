import numpy as np
import pytest
from operations import common_outer_step20 as step
...
def test_source_acceptance_cannot_be_replaced(field,value):
    record=dict(accepted_outer_steps=19,previous_accepted_outer_steps=18,
        accepted_target_is_response=False,coupled_column_converged=False,
        physical_time_advanced=False,job_id=76554,
        candidate=step.SOURCE+'/confirm2/trial_material.npz')
    terminal=dict(job_id=76554,state='COMPLETED')
...
def test_failed_scheduler_is_not_an_accepted_source():
    record=dict(accepted_outer_steps=19,previous_accepted_outer_steps=18,
        ...
        physical_time_advanced=False,job_id=76554,
        candidate=step.SOURCE+'/confirm2/trial_material.npz')
    with pytest.raises(RuntimeError):step.validate_acceptance(record,dict(job_id=76554,state='FAILED'))


def test_step20_does_not_reuse_old_amplitude_or_direction():
    accepted,old=fixture();r=np.arange(8)*.001
