...
import numpy as np
import pytest
from operations import common_step21_probe as step
...
@pytest.mark.parametrize('alpha',[0.,1/256])
def test_backtracking_changes_only_numeric_displacement(alpha):
    base,old=fixture();r=base['base_residual'];t=step.make_trial(base,r,old,alpha)
    assert np.array_equal(t['encoded_state'],base['encoded_state']+alpha*r)
...
def test_wrong_candidate_contract_is_rejected(mutation):
    base,old=fixture();r=base['base_residual'].copy();t=step.make_trial(base,r,old,1/256)
    if mutation=='amplitude':t['relaxation']=np.array(1/64)
    if mutation=='direction':t['finite_direction']*=2
    ...
    if mutation=='base':base['relaxation']=np.array(1/128)
    with pytest.raises(ValueError):step.exact_trial(t,base,r,old,1/256)


def test_undeclared_or_trust_violating_trial_never_initializes():
    base,old=fixture();r=base['base_residual'].copy()
    with pytest.raises(ValueError):step.make_trial(base,r,old,1/512)
    r*=10000;base['base_residual']=r.copy();base['finite_direction']=r.copy()
    with pytest.raises(ValueError):step.make_trial(base,r,old,1/256)


@pytest.mark.parametrize('mode,expected',[
    ('baseline_unstable',['control']),('physical_domain_rejected',['control','half']),
    ('unresolved_or_other_rejection',['control','half']),('supported',['control','half']),
    ('contraction_rejected',['control','half']),
])
def test_no_extra_amplitude_after_unresolved_or_supported_result(mode,expected):
    seen=[]
    def evaluate(name):
        seen.append(name)
        if name=='control':return mode if mode=='baseline_unstable' else 'baseline_stable'
        return mode
    step.amplitude_sequence(evaluate);assert seen==expected


def test_smaller_amplitude_requires_resolved_contraction_failure():
    gates={k:True for k in REQUIRED_GATES};gates['candidate_mass_weighted_contraction_pass']=False
    r=dict(feedback_evaluated=True,physical_response_failures={},pair_supported=False,original_gates=gates,fresh_baseline_comparison=dict(passed=False))
    assert step.permits_smaller(r)
    for key in ('last_two_atomic_heating_pass','inner_noise_resolved_pass','population_nonnegative_pass','two_inner_radiation_residuals_pass'):
        bad=deepcopy(r);bad['original_gates'][key]=False;assert not step.permits_smaller(bad)
    for key,value in [('feedback_evaluated',False),('physical_response_failures',{'final':'invalid'}),('pair_supported',True)]:
        bad=deepcopy(r);bad[key]=value;assert not step.permits_smaller(bad)
    bad=deepcopy(r);bad['original_gates'].pop('two_boundary_spectra_pass');assert not step.permits_smaller(bad)
...
def test_only_one_probe_after_resolved_contraction_rejection():
    seen=[]
    def evaluate(name):
        seen.append(name)
        return 'baseline_stable' if name=='control' else 'contraction_rejected'
    assert step.amplitude_sequence(evaluate)=='half_rejected_requires_direction_review'
    assert seen==['control','half']
    assert step.ALPHAS=={'control':0.,'half':1/256}
...
def test_probe_requires_audited_resolved_maximum_cell_failure(change):
    gates={k:True for k in REQUIRED_GATES};gates['candidate_maximum_cell_contraction_pass']=False
    audit={'accepted_outer_steps':20,'pairs':{'pair04':{},'pair08':{'gate_checks':gates,'fresh_baseline_comparison':{'passed':False}}}}
    terminal={'job_id':76808,'state':'COMPLETED'}
    ...
    if change=='cross':audit['pairs']['pair08']['fresh_baseline_comparison']['passed']=True
