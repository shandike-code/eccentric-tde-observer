...
from operations import bounded_steps13_to15 as task
...
def test_configuration_tracks_actual_source_index_and_restores_on_error(monkeypatch):
    ...
    with pytest.raises(RuntimeError,match='injected'):
        with task.sequence_contract():
            assert seq.FIRST_INDEX==13 and seq.SOURCE==task.SOURCE
            assert stage.batch.ALPHAS=={'full':1/64,'half':1/128}
            ...
            with pytest.raises(ValueError):stage.configure('invalid',16)
...
@pytest.mark.parametrize('failed_cycle',[None,13,14,15])
def test_conditional_three_cycles_and_final_confirmation(monkeypatch,tmp_path,failed_cycle):
    ...
    expected=list(range(13,16 if failed_cycle is None else failed_cycle+1))
    ...
    if failed_cycle is None:
        assert events[-1]==('tail',15) and status=='complete_requires_review'
    else:
