...
import numpy as np
import pytest
from operations import validate_step21_tapered_joint as valid
from operations import tapered_joint_fields as fields
from test_recover_step21_control_windows import templates,endpoints


def arrays(tmp_path):
    shape=(1024,2,3);x=np.ones(shape);x[950]=np.nextafter(0.,1.)
    path=tmp_path/'source.dat';x.tofile(path);repl={}
    for i in (1,5):
        p=tmp_path/f'block{i}.npz';np.savez(p,candidate=np.full((384,2,3),2.));repl[i]=p
...
def test_saved_candidates_untouched_subnormal_and_affine_operator(tmp_path):
    ...
    fields.write_candidates(p,repl,qpath,hpath,shape,centers=(1,5))
    q=np.fromfile(qpath).reshape(shape);h=np.fromfile(hpath).reshape(shape)
    assert np.array_equal(q[896:],x[896:]) and np.array_equal(h[896:],x[896:])
    assert np.array_equal(h[:384],.5*x[:384]+.5*q[:384])
    ...
    report=fields.validate(paths,shape,blocks=(0,1,2,4,5,6))
    assert report['all_groups_evaluated']==1024 and report['fixed_scale_l2_ratios'][3]<1e-14
    assert sum(row['group_count'] for row in report['slabs'])==1024
    assert report['slabs'][12]['output_change_linf']>0  # 未改输入的邻块输出会改变
    bad=np.fromfile(paths[2]).reshape(shape);bad[950]=1.;bad.tofile(paths[2])
    with pytest.raises(ValueError,match='unselected'):fields.validate(paths,shape,blocks=(0,1,2,4,5,6))
...
def test_candidate_failure_preserves_source_and_never_completes(tmp_path,bad):
    ...
    if bad in ('negative','nan','shape'):
        a=np.ones((384,2,3) if bad!='shape' else (383,2,3));a.flat[0]=-1. if bad=='negative' else (np.nan if bad=='nan' else 1.)
        np.savez(repl[1],candidate=a)
    if bad=='inventory':repl.pop(1)
    ...
    with pytest.raises((ValueError,FileExistsError,valid.reused.Stopped)):
        fields.write_candidates(p,repl,q,h,shape,centers=(1,5),checkpoint=callback)
...
def test_sequence_full_half_then_ten_control_maps():
    ...
    assert result=='tapered_joint_validation_complete_requires_review'
...
@pytest.mark.parametrize('centers',[(1,2),(0,5),(1,8),(1,1),(True,5)])
def test_overlapping_or_outside_joint_cores_rejected(centers):
    with pytest.raises(ValueError):fields.intervals(centers,1024)


def test_taper_is_fixed_symmetric_convex_and_not_commuting_with_frequency_map(tmp_path):
    shape,x,p,repl=arrays(tmp_path);qp=tmp_path/'q.dat';hp=tmp_path/'h.dat'
    fields.write_candidates(p,repl,qp,hp,shape,centers=(1,5))
    q=np.fromfile(qp).reshape(shape);w=fields.taper_weights()
    assert np.all((w>0)&(w<1)) and w[0]<2e-5 and w[191]>.9999
    np.testing.assert_allclose(w,w[::-1],rtol=1e-11)
    expected=np.sin(np.pi*(np.arange(384)+.5)/384.)**2
    np.testing.assert_array_equal(q[:384],(1-expected[:,None,None])*x[:384]+expected[:,None,None]*2)
    np.testing.assert_array_equal(q[512:896],q[:384])
    # A frequency-dependent taper and neighbor-coupled operator do not commute.
    uniform=x.copy();uniform[:384]=2.;uniform[512:896]=2.
    all_w=np.zeros(shape[0]);all_w[:384]=w;all_w[512:896]=w
    op=lambda a:.25*a+.05*np.roll(a,1,axis=0)+1.
    wrong=(1-all_w[:,None,None])*op(x)+all_w[:,None,None]*op(uniform)
    assert np.max(abs(op(q)-wrong))>1e-4


def test_real_twenty_percent_cost_gate_without_changing_old_protocol():
    from operations import composite_hybrid_audit as old
    report={'fixed_scale_l2_ratios':[1.,.81,.9,1e-8],'fixed_scale_linf_ratios':[1.,.9,.95,1e-8]}
    row={'residual':1e-5,'boundary_l1':1e-5,'boundary_bolometric':1e-5}
    assert old.GATES['full_l2_ratio_max']==.999
    assert not fields.checks(report,row,row,row)['full_l2_benefit']
    report['fixed_scale_l2_ratios'][1]=.8
    assert all(fields.checks(report,row,row,row).values())
