...
import numpy as np
import pytest
from operations import common_confirmation_step20 as step
...
def test_zero_feedback_uses_step20_postcheck_without_trial_acceptance(monkeypatch,tmp_path):
    ...
    seen=[];monkeypatch.setattr(step,'verify_step20_pair',lambda folder,p,result:seen.append(result))
...
def test_protocol_keeps_step19_direction_and_counter(monkeypatch):
    monkeypatch.setattr(step.fresh,'new_protocol',lambda *a:{'sources':{}})
    p=step.confirmation_protocol({},None,{},[],{},False)
    assert p['numerical_backtracking']==dict(alpha=1/128,fixed_base_accepted_index=19,fixed_direction='r19',physical_time_advanced=False)
