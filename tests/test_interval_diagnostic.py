...
def test_registered_round_survives_a_missing_directory(run_dir, monkeypatch):
    """start_round persists ownership BEFORE mkdir, so a crash between the two
    leaves a registered intent whose directory is legitimately recreated."""
    state = read_state(run_dir)
    state["history"] = [history_row(3), history_row(4)]
    driver.start_round(run_dir, {}, state, run_dir / "state.json")
    shutil.rmtree(run_dir / "feedback-round1")          # the crash window
    assert read_state(run_dir)["pending_feedback"]["round"] == 1

    def stop_here(*a, **k):
        raise RuntimeError("reached protocol build")

    monkeypatch.setattr(driver, "build_round_protocol", stop_here)
    with pytest.raises(RuntimeError, match="reached protocol build"):
        driver.complete_round(run_dir, {}, state, run_dir / "state.json")
    assert (run_dir / "feedback-round1").is_dir()        # recreated from the intent


def test_registered_late_stage_round_without_artifacts_is_refused(run_dir):
    """Past the protocol stage the artifacts are the evidence; a missing directory
    is then a real inconsistency, not a recoverable window."""
    state = read_state(run_dir)
    state["history"] = [history_row(3), history_row(4)]
    pending = driver.start_round(run_dir, {}, state, run_dir / "state.json")
    shutil.rmtree(run_dir / "feedback-round1")
    pending["stage"] = "ledger"
    with pytest.raises(RuntimeError, match="registered feedback artifacts are missing"):
        driver.complete_round(run_dir, {}, state, run_dir / "state.json")


def test_unregistered_round_directory_is_refused(run_dir):
    """start_round only opens new rounds; an existing directory there is a conflict."""
    state = read_state(run_dir)
    state["history"] = [history_row(3), history_row(4)]
    (run_dir / "feedback-round1").mkdir()
    with pytest.raises(RuntimeError, match="round directory already exists"):
        driver.start_round(run_dir, {}, state, run_dir / "state.json")
