...
import numpy as np
import pytest
from operations import validate_step21_five_blocks as valid
...
def arrays(tmp_path):
    shape=(1792,2,3);x=np.ones(shape);x[1700]=np.nextafter(0.,1.)
    path=tmp_path/'source.dat';x.tofile(path);repl={}
    for i in (2,8):
        p=tmp_path/f'block{i}.npz';np.savez(p,candidate=np.full((640,2,3),2.));repl[i]=p
...
def test_saved_candidates_untouched_subnormal_and_affine_operator(tmp_path):
    ...
    fields.write_candidates(p,repl,qpath,hpath,shape,centers=(2,8))
    q=np.fromfile(qpath).reshape(shape);h=np.fromfile(hpath).reshape(shape)
    assert np.array_equal(q[1408:],x[1408:]) and np.array_equal(h[1408:],x[1408:])
    assert np.array_equal(h[:640],.5*x[:640]+.5*q[:640])
    ...
    report=fields.validate(paths,shape,blocks=(0,1,2,3,4,6,7,8,9,10))
    assert report['all_groups_evaluated']==1792 and report['fixed_scale_l2_ratios'][3]<1e-14
    assert sum(row['group_count'] for row in report['slabs'])==1792
    assert report['slabs'][20]['output_change_linf']>0  # 未改输入的邻块输出会改变
    bad=np.fromfile(paths[2]).reshape(shape);bad[1700]=1.;bad.tofile(paths[2])
    with pytest.raises(ValueError,match='unselected'):fields.validate(paths,shape,blocks=(0,1,2,3,4,6,7,8,9,10))
...
def test_candidate_failure_preserves_source_and_never_completes(tmp_path,bad):
    ...
    if bad in ('negative','nan','shape'):
        a=np.ones((640,2,3) if bad!='shape' else (639,2,3));a.flat[0]=-1. if bad=='negative' else (np.nan if bad=='nan' else 1.)
        np.savez(repl[2],candidate=a)
    if bad=='inventory':repl.pop(2)
    ...
    with pytest.raises((ValueError,FileExistsError,valid.reused.Stopped)):
        fields.write_candidates(p,repl,q,h,shape,centers=(2,8),checkpoint=callback)
...
def test_sequence_full_half_then_ten_control_maps():
    ...
    assert result=='five_block_global_validation_complete_requires_review'
...
@pytest.mark.parametrize('centers',[(2,4),(0,8),(2,14),(2,2),(True,8)])
def test_overlapping_or_outside_joint_cores_rejected(centers):
    with pytest.raises(ValueError):fields.intervals(centers,1792)
