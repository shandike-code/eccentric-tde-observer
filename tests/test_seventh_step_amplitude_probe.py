...
import numpy as np
import pytest
from operations import seventh_step_amplitude_probe as probe
...
def test_amplitudes_are_scoped_and_restored_on_failure():
    ...
    with pytest.raises(RuntimeError):
        with probe.amplitude_contract():
            assert probe.batch.ALPHAS == {'full': 1/128, 'half': 1/256}
...
def test_seventh_protocol_adapter_retains_all_gates_and_physical_claims(monkeypatch):
    ...
    with probe.amplitude_contract():
        t=make_trial(base,r,1/128);validate_trial(t,base,r)
        assert np.array_equal(t['encoded_state'],x+r/128)
        for k in ('density_g_cm3','phase_index','step_duration_s'):assert np.array_equal(t[k],base[k])
        with pytest.raises(ValueError,match='undeclared'):make_trial(base,r,1/64)
        t['base_residual']=r*2
...
def test_no_acceptance_cannot_claim_next_stage(monkeypatch,tmp_path):
    ...
    assert d['requires_review_before_confirmation_or_step8'] and not d['coupled_column_accepted']
...
def test_formal_acceptance_with_failed_fresh_control_does_not_corroborate(monkeypatch,tmp_path):
    ...
    assert d['requires_review_before_confirmation_or_step8']
