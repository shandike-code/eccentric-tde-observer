...
import numpy as np
import pytest
from operations import twelfth_step_amplitude_probe as probe
...
def test_twelfth_protocol_adapter_retains_all_gates_and_physical_claims(monkeypatch):
...
def test_no_acceptance_cannot_claim_next_stage(monkeypatch,tmp_path):
    ...
    assert d['requires_review_before_confirmation_or_step13'] and not d['coupled_column_accepted']
...
def test_native_preflight_requests_one_actual_process_not_four_workers(monkeypatch):
    from operations import step12_native_preflight as native
    calls=[]
...
def test_formal_acceptance_with_failed_fresh_control_does_not_corroborate(monkeypatch,tmp_path):
    ...
    assert d['requires_review_before_confirmation_or_step13']


def test_source_is_the_reviewed_eleventh_step_and_preflight_checks_claims(monkeypatch):
    calls=[]
    ...
    assert calls==[('outputs/hpc/outer-steps9to11-20260922/step11/next-step',11),['claim']]
