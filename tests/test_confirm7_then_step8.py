import numpy as np
import pytest
from operations import confirm7_then_step8 as task


def materials(alpha=1/128):
...
def test_actual_amplitude_and_displacement_are_both_required():
    ...
    with pytest.raises(RuntimeError):task.validate_accepted_material(*materials(1/256))
...
def test_adapters_restore_frozen_module_bindings_after_exception():
    ...
    with pytest.raises(RuntimeError):
        with task.adapters():
            ...
            assert task.batch.ALPHAS=={'full':1/128,'half':1/256}
            raise RuntimeError('injected')
    assert task.stage.prepare_confirmation is a and task.batch.ALPHAS is b


@pytest.mark.parametrize('field,value',[
    ('outer_step_index',6),('formal_finite_step_accepted',False),
    ('fresh_control_corroborated',False),('alphas',{'full':1/256,'half':1/512})])
def test_unqualified_source_never_runs_confirmation(monkeypatch,tmp_path,field,value):
    d={'outer_step_index':7,'formal_finite_step_accepted':True,
...
def test_validated_source_uses_bounded_combined_driver(monkeypatch,tmp_path):
    ...
    assert calls==[(task.SOURCE,7),'execute']
