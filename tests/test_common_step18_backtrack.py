...
import numpy as np
import pytest
from operations import common_step18_backtrack as step
...
@pytest.mark.parametrize('alpha',[0.,1/128,1/256])
...
def test_wrong_candidate_contract_is_rejected(mutation):
    base,old=fixture();r=base['base_residual'].copy();t=step.make_trial(base,r,old,1/128)
...
def test_undeclared_or_trust_violating_trial_never_initializes():
    ...
    with pytest.raises(ValueError):step.make_trial(base,r,old,1/128)
...
@pytest.mark.parametrize('mode,expected',[
    ...
    ('contraction_rejected',['control','half','quarter']),
])
...
def test_zero_control_does_not_call_finite_step_acceptance(monkeypatch,tmp_path):
    ...
    seen=[];monkeypatch.setattr(step,'verify_step18_pair',lambda folder,p,result:seen.append(result))
