import numpy as np
import pytest
from operations import scan_step21_joint_line_cost as scan
...
def test_safety_fraction_and_exact_quadratic():
    ...
    assert r['predicted_l2_ratio']==pytest.approx(np.sqrt(1-.09+.09**2)) and not r['l2_cost_pass']
...
def test_cost_gate_rejects_small_gain_and_accepts_useful_gain():
    assert scan.CONTROLS['minimum_l2_gain']==.2
    weak=scan.choose(1.,1.,-.5,.1)
    assert weak['fraction']>0 and not weak['l2_cost_pass']
    strong=scan.choose(1.,1.,-1.,1.)
    assert strong['l2_cost_pass'] and strong['predicted_l2_ratio']==pytest.approx(.1)
    assert strong['best_feasible_l2_ratio']==0


@pytest.mark.parametrize('delta',[.3,-.3,.08])
def test_real_scan_witness_json_serialization(tmp_path,monkeypatch,delta):
    import json
    from test_scan_step21_block_short_step_serialization import exercise
    r=exercise(scan,tmp_path,monkeypatch,delta)
    assert json.loads(json.dumps(r,allow_nan=False))==r
    assert not r['actual_map_performed'] and not r['candidate_written']
