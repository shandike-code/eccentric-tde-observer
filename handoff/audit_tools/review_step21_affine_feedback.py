"""Independent six-endpoint audit after measured affine radiation acceleration."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,prior_directions,prior_maps,output):
    ...
    if complete:
        root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==20 and root['status']=='bounded_affine_diagnostic_complete_requires_review'
    else:
    ...
    # 固定物质与76931逐字节相同；初始种子改为已独立检验的全局外推候选。
    ...
    for child in ('control','thermal','population'):
        ...
        assert declaration['basis'][child]['basis']==[read(prior_directions/child/'endpoints-map08/manifest.json')['endpoints'][k] for k in ('previous','final','mapped_final')]
    ...
    drift={child:asdict(encoded_residual_norms(residuals[child+'03']['final']-arrays(prior_directions/child/'pair08/final_response.npz')['residual'],mass)) for _,child,_ in selected}
    ...
    fig.suptitle('Step21 after affine acceleration: original gates unchanged');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
