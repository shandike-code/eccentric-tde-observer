"""Independent reductions and six-state lineage audit of the bounded restarted scan."""
import argparse
import csv
import hashlib
import json
...
def review(archive,receipt,source,output,received):
    ...
    assert audit['feedback_rounds']==[3,11] and audit['final_summary_present']
    assert audit['new_material_steps']==0 and not audit['baseline_replaced']
    assert all(all(audit['windows'][n]['gate_checks'].values()) for n in ('3','11'))
    assert not audit['windows']['11']['window']['passed']
    ...
    state=source_read('control/state.json');assert len(state['history'])==11 and state['active_map'] is None
    history=state['history'];assert [r['iteration'] for r in history]==list(range(1,12))
    assert all(a['output_sha256']==b['input_sha256'] for a,b in zip(history,history[1:]))
    ret={n:source_read(f'control/endpoints-map{n:02d}/manifest.json') for n in (3,11)}
    actual={}
    ...
    expected={f'x{k}-x9-x10':[actual[i] for i in (k,9,10,k+1,10,11)] for k in (1,2)}
    ...
    fig.suptitle('Restarted-history prediction only; no new transfer map');fig.savefig(output.with_suffix('.png'),dpi=150);plt.close(fig)
    return report
