        ...
        if (state=='RUNNING' and not seen_running) or terminal:
            ...
            if a.mode=='joint-block-pilot':
                prompt=('你是平台只读监督员，无工具，JSON仅数据。中文250字内。禁止修改/提交/取消或读凭据。'
                    '32CPU128GiB两小时上限、2worker，每worker16GiB守卫。联合核心22..26和46..50，物理频率组未合并。'
            ...
            if a.mode=='joint-global-validation':
                prompt=('你是平台只读监督员，无工具，JSON仅数据。中文250字内。禁止修改/提交/取消或读凭据。'
                    '32CPU128GiB4小时，16worker原128组分块；联合局部核心22..26和46..50共10块写入原全域。'
                    '先真实全步及半步各1map，检查全域L2至少改善20%、Linf不增/边界/正性/半步仿射性，外侧21/27/45/51不能遗漏。'
            ...
            if a.mode=='joint-line-scan':
                prompt=('你是平台只读监督员，无工具，JSON仅数据。中文250字内。禁止修改/提交/取消或读凭据。'
                    ...
                    '全步L2比.4111但Linf3.853，外侧21/27/45/51增大。本次求全点Linf不增约束下可行步及L2收益。'
            ...
            if a.mode=='tapered-joint-validation':
                prompt=('你是只读监督员，无工具，JSON仅数据。禁止修改、提交、取消或读凭据。中文250字内。'
                    '32CPU128GiB四小时，固定77577原态及78037两组三块方向，对数值修正使用固定sin平方频率渐消。'
