"""Six-endpoint audit after positive-plane validation; requires all three feedback pairs."""
import argparse
...
def review(archive,receipt,out,reference,physical_old,prior_confirmed,prior_directions,prior_maps,output):
    ...
    if complete:
        root=read(out/'summary.json');assert root['new_material_steps']==0 and root['accepted_outer_steps']==20 and root['status']=='bounded_positive_plane_diagnostic_complete_requires_review'
    else:
        assert manifest['stage'] in ('control-feedback','thermal-feedback','population-feedback')
        root={'cases':{c:read(out/c/'pair03/decision.json') for c in ('control','thermal','population') if (out/c/'pair03/decision.json').exists()}}
    selected=[(c+'03',c,3) for c in ('control','thermal','population') if c in root['cases']]
    assert selected and selected[0][1]=='control'
    if complete:assert len(selected)==3
    results={};residuals={};peaks=[];formal='source_formal_heating_erg_s_cm3'
    fig,axes=plt.subplots(1,3,figsize=(14,4),layout='constrained')
    for case,child,n in selected:
        ...
        # 只投影候选位移；反馈与残差始终保留全部四分量。
        if child=='thermal':direction[:,1:]=0
        if child=='population':direction[:,0]=0
        direction=direction.ravel()
        assert np.array_equal(t['base_encoded_state'],base['encoded_state']) and np.array_equal(t['base_residual'],r) and np.array_equal(t['finite_direction'],direction)
        assert float(t['relaxation'])==(0 if child=='control' else 1/256)
        assert np.array_equal(t['encoded_state'],base['encoded_state']+float(t['relaxation'])*direction)
        if child=='control':
            assert set(t)==set(base) and all(np.array_equal(t[k],base[k]) for k in t)
        else:
            assert np.array_equal(t['finite_direction'],direction)
            frozen=arrays(out/'inputs'/child/'trial_material.npz');assert set(t)==set(frozen) and all(np.array_equal(t[k],frozen[k]) for k in t)
        ...
        state=read(out/child/'state.json');assert len(state['history'])==3 and state['active_map'] is None
        ...
        assert digest(out/'inputs'/child/'trial_material.npz')==digest(out/child/'trial_material.npz')
        ...
        if child=='control':
            ...
            failed={k for k,v in s['gate_checks'].items() if not v}
            assert s['decision']['finite_trial_accepted_as_one_nonlinear_step']==(not failed)
            ...
            cross=cross_comparisons(vectors,residuals[f'control{n:02d}'],mass)
            assert cross['passed']==root['cases'][child]['fresh_baseline_comparison']['passed']
            assert root['cases'][child]['pair_supported']==(not failed and cross['passed'])
            saved=root['cases'][child]['fresh_baseline_comparison']['ratios']
            for k,v in cross['ratios'].items():
                for metric,x in v.items():assert np.isclose(x,saved[k][metric],rtol=1e-12,atol=0)
        ...
        results[case]={'endpoints':ends,'comparison':s['comparison'],'all_16_pair_gates':child!='control' and all(s['gate_checks'].values()),'gate_checks':s['gate_checks']};residuals[case]=vectors
        if child!='control':results[case]['fresh_baseline_comparison']=cross
        axes[0].plot(np.linalg.norm(vectors['final'].reshape(128,4),axis=1),label=case)
    # 零控制保持已接受x20本身，不使用其响应目标替换物质。
    ...
    # 固定物质与76957逐字节相同；新四态正性候选已另经首张真实map审计。
    declaration=read(out/'declaration.json')
    prior_inventory={c['path']:c for c in read(prior_directions/'ARCHIVE_MANIFEST.json')['files']}
    for child in ('control','thermal','population'):
        for name in ('trial_material.npz','state.json'):
            f=prior_directions/child/name;c=prior_inventory[child+'/'+name]
            assert f.stat().st_size==c['size_bytes'] and digest(f)==c['sha256']
        assert digest(out/child/'trial_material.npz')==digest(prior_directions/child/'trial_material.npz')
        validation=read(out/child/'validation.json')
        assert read(out/child/'state.json')['history'][0]==validation['actual_map']
        assert read(out/child/'state.json')['history'][0]['input_sha256']==validation['candidate']['sha256']
        assert validation['validated'] and all(validation['checks'].values())
        old_validation=prior_maps/child/'validation.json'
        assert digest(out/child/'validation.json')==digest(old_validation)
        assert declaration['basis'][child]['basis']==[read(prior_directions/child/'validation.json')['candidate']]+[read(prior_directions/child/'endpoints-map03/manifest.json')['endpoints'][k] for k in ('previous','final','mapped_final')]
        assert validation['effective_uv']==declaration['prediction'][child]['rounds'][-1]['result']['uv']
        assert declaration['prediction'][child]['feasible']
    assert not (out/'quarter').exists()
    for child in ('control','thermal','population'):
        history=read(out/child/'state.json')['history']
    ...
    drift={child:asdict(encoded_residual_norms(residuals[child+'03']['final']-arrays(prior_directions/child/'pair03/final_response.npz')['residual'],mass)) for _,child,_ in selected}
    for c in declaration['code']:
        f=Path(c['path']);assert f.stat().st_size==c['size_bytes'] and digest(f)==c['sha256']
    baseline_drift={c:asdict(encoded_residual_norms(v['final']-v['previous'],mass)) for c,v in residuals.items()}
    control_reference_drift=asdict(encoded_residual_norms(residuals['control03']['final']-np.load(reference/'base_residual.npy',allow_pickle=False),mass))
    for ax,k,limit in [(axes[1],'atomic_heating_volume_l1',.001),(axes[2],'inner_noise_to_trial_signal_l2_ratio',.1)]:
        plotted={c:v for c,v in results.items() if k in v['comparison']};ax.bar(list(plotted),[v['comparison'][k]/limit for v in plotted.values()]);ax.axhline(1,color='black');ax.set(title=k,ylabel='Metric / original gate')
    for ax in axes[1:]:ax.tick_params(axis='x',labelrotation=25)
    axes[0].set(xlabel='Half-column cell',ylabel='Encoded residual cell norm');axes[0].legend()
    fig.suptitle('Step21 positive-plane validation: original gates unchanged');fig.savefig(output.with_suffix('.png'),dpi=160);plt.close(fig)
    record={'archive':read(receipt),'final_summary_present':complete,'audited_cases':[v[1] for v in selected],'verified_files':len(manifest['files']),'pairs':results,'feedback_process_receipts':feedback_receipts,'map_process_receipts':len(peaks)-feedback_receipts,'total_map_wall_s':sum(times),'maximum_proc_kib':max(peaks),
        'same_material_pre_to_post_positive_plane_drift':drift,'verified_code_claims':len(declaration['code']),'baseline_pair_drift':baseline_drift,'current_control_minus_r20':control_reference_drift,'drift_is_error_bound':False,'material_response_recomputed_on_mac':False,'accepted_outer_steps':20,'cross_platform_decode_rtol':8*np.finfo(float).eps}
    output.with_suffix('.json').write_text(json.dumps(record,indent=2,allow_nan=False)+'\n');print(json.dumps(record,indent=2))


def main():
    p=argparse.ArgumentParser()
    for k in ('archive','receipt','received','reference','physical-old','prior-confirmed','prior-directions','prior-maps','output'):p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();review(a.archive,a.receipt,a.received,a.reference,a.physical_old,a.prior_confirmed,a.prior_directions,a.prior_maps,a.output)
