"""Audit convex-direction formal feedback and its frozen-reference eight-map window."""
...
def review(archive,receipt,received,first,prior,reference,physical_old,output):
    ...
    for root,name in [(first,'20260927-convex-joint-first-review'),(prior,'20260926-heating-validation-complete-review')]:
    ...
    for n in selected:
        ...
        for key,path in [('control_window_declaration',out/'declaration.json'),('convex_joint_validation',out/'control/validation.json'),
    ...
    first_audit=read(Path('handoff/evidence/20260927-convex-joint-first-review.json'));peaks.append(first_audit['maximum_worker_peak_kib'])
    ...
    if complete:
        ...
        if s['status']=='convex_joint_validation_complete_requires_review':assert len(maps)==10 and selected==[2,10] and reports['10']['window']['passed']
    ...
    fig.suptitle('Convex-direction feedback audit; no new material acceptance');fig.savefig(output.with_suffix('.png'),dpi=150);plt.close(fig)
    return result
