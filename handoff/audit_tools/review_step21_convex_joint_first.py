"""Independent first-stage 78032 convex map audit; feedback remains pending."""
...
def main():
    root=Path('outputs/review-20260925');out=root/'convex-joint-78032-first-received'
    receipt=root/'full-half-validation-1790451595322386962-receipt.json'
    manifest=receive(root/'full-half-validation-1790451595322386962.tar.gz',receipt,out)
    prior=root/'heating-validation-77577-complete-received';local=root/'joint-blocks-77817-received'
    for source,ev in [(prior,'20260926-heating-validation-complete-review'),(local,'20260926-joint-block-pilot-review')]:
    ...
    terminal=read(Path('handoff/evidence/20260927-convex-joint-78032-first-observation.json'))
    assert terminal['job_id']==78032 and 'JobState=RUNNING' in terminal['scontrol']
    ...
    scan=root/'joint-taper-plane-78031-received';scan_audit=read(Path('handoff/evidence/20260927-joint-taper-plane-review.json'))
    ...
    verified_inventory(scan);prediction=read(scan/'prediction.json')['rounds'][-1]['result'];sd=read(scan/'declaration.json')
    assert d['prediction']==prediction and d['uv']==scan_audit['selected_uv']==prediction['uv']
    assert d['convex_basis']==[sd['paths'][i] for i in (0,2,4)] and sd['source_row']==d['source_row']
    assert d['selected']==val['selected']=='convex-joint-and-tapered-directions' and d['gates']['full_l2_ratio_max']==.8
    assert scan_audit['eligible_for_independent_review'] and d['prediction_ratio_absolute_tolerance']==1e-6
    ...
    errors=[abs(l2[1]-prediction['l2_ratio']),abs(linf[1]-prediction['linf_ratio'])]
    checks['predicted_full_l2_reproduced']=errors[0]<=1e-6
    checks['predicted_full_linf_reproduced']=errors[1]<=1e-6
    ...
    result=dict(archive=read(receipt),verified_files=len(manifest['files']),verified_code_claims=len(d['code']),process_receipts=len(peaks),maximum_worker_peak_kib=max(peaks),accepted_outer_steps=20,new_material_steps=0,maps=2,feedback_pairs=0,
        fixed_scale_l2_ratios=l2,fixed_scale_linf_ratios=linf,checks=checks,validated=True,prediction_absolute_errors=errors,worst_full_slabs=ranked,feedback_pending=True,
    ...
    target=Path('handoff/evidence/20260927-convex-joint-first-review');target.with_suffix('.json').write_text(json.dumps(result,indent=2)+'\n')
    ...
    ax.axhline(1.,color='black',ls='--');ax.set(xlabel='Frequency block',ylabel='Block maximum defect / original GLOBAL maximum',title='Real convex candidate passed global gates; feedback pending');ax.legend()
