"""Independent reduction of 77701 full/half map evidence, including all neighbors."""
...
def main():
    root=Path('outputs/review-20260925');out=root/'heating-block-global-77701-received'
    receipt=root/'complete-1790414187041810474-receipt.json'
    manifest=receive(root/'complete-1790414187041810474.tar.gz',receipt,out)
    ...
    terminal=read(Path('handoff/evidence/20260926-heating-block-global-77701-terminal.json'))
    assert terminal['job_id']==77701 and terminal['state']=='COMPLETED' and 'ExitCode=0:0' in terminal['scontrol']
    ...
    target=Path('handoff/evidence/20260926-block-global-review');target.with_suffix('.json').write_text(json.dumps(result,indent=2)+'\n')
