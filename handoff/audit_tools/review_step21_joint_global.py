"""Independent reduction of 77843 full/half map evidence, including all neighbors."""
...
def main():
    root=Path('outputs/review-20260925');out=root/'joint-block-global-77843-received'
    receipt=root/'complete-1790430251654118312-receipt.json'
    manifest=receive(root/'complete-1790430251654118312.tar.gz',receipt,out)
    ...
    assert d['selected']=='joint-cores23-to25-and47-to49-full'
    assert val['field_comparison']['selected_blocks']==[23,24,25,47,48,49]
    terminal=read(Path('handoff/evidence/20260926-joint-global-77843-terminal.json'))
    assert terminal['job_id']==77843 and terminal['state']=='COMPLETED' and 'ExitCode=0:0' in terminal['scontrol']
    ...
    target=Path('handoff/evidence/20260926-joint-global-review');target.with_suffix('.json').write_text(json.dumps(result,indent=2)+'\n')
    ...
    ax.axhline(1.,color='black',ls='--');ax.set(xlabel='Frequency block',ylabel='Block maximum defect / original GLOBAL maximum',title='Local correction amplifies neighboring block defects');ax.legend()
