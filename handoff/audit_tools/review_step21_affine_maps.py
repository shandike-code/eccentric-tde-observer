"""Mac-only small-artifact audit, outside the running job's numerical modules."""
import argparse
import hashlib
import json
...
def review(archive,receipt,received,prior,scan,output):
    ...
    for case in ('control','thermal','population'):
        ...
        assert s['active_map'] is None and len(s['history'])==1 and s['history'][0]==v['actual_map']
        ...
        assert v['candidate']['size_bytes']==10099884032 and v['fraction']==plan['prediction'][case]['direction']['selected_forward_fraction']
        ...
        prior_res=plan['prediction'][case]['latest_actual_residual']
        ...
        assert checks==v['checks'] and all(checks.values()) and v['validated'] is True
