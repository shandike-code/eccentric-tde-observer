...
def main():
    root=Path('outputs/review-20260925');archive=root/'complete-1790409114970558201.tar.gz';receipt=root/'complete-1790409114970558201-receipt.json'
    out=root/'heating-blocks-77648-received';manifest=receive(archive,receipt,out)
    ...
    terminal=read(Path('handoff/evidence/20260926-heating-block-pilot-77648-terminal.json'));assert terminal['job_id']==77648 and terminal['state']=='COMPLETED' and 'ExitCode=0:0' in terminal['scontrol']
    ...
    assert (d['gmres_restart'],d['gmres_cycles'],d['gmres_tolerance'],d['maximum_source_maps_per_case'])==(8,2,1e-5,3)
    ...
    for i in (24,48):
        ...
        assert proc['returncode']==0 and proc['memory_guard_passed'] and proc['native_observed_peak_kib']<6*1024**2
        assert not (out/f'control-block{i:02d}.err').read_text()
        claim=r['local_arrays'];p=root/Path(claim['path']).name
        ...
        assert a.shape==b.shape==(128,32,4096) and all(np.isfinite(x).all() and np.all(x>=0) for x in (a,b))
        ...
        old=read(prior/f'control/map0010/block{i:02d}.json')
        assert_close(r['raw_defect_linf'],old['maximum_absolute_radiation_change'])
        assert_close(r['raw_boundary_absolute'],old['boundary_spectrum_l1_numerator'])
        ...
        checks=dict(l2_halved=l2ratio<=.5,linf_halved=maxratio<=.5,
            ...
            memory_below_6gib=r['peak_rss_mib']<6144,time_below_20_maps=r['wall_s']<=20*r['raw_map_s'])
        assert checks==r['checks'] and r['eligible_for_further_block_review']==all(checks.values())
        assert r['core_groups']==[i*128,(i+1)*128] and not r['global_candidate_written'] and not r['formal_acceptance_changed']
        rows.append(dict(block=i,local_arrays=claim,l2_ratio=l2ratio,linf_ratio=maxratio,checks=checks,
            local_candidate_min=float(a.min()),local_mapped_min=float(b.min()),raw_map_s=r['raw_map_s'],wall_s=r['wall_s'],
    ...
    result=dict(archive=read(receipt),verified_files=len(manifest['files']),verified_code_claims=len(d['code']),rows=rows,
        ...
        operator_recomputed_on_mac=False,global_validation_required=True)
    target=Path('handoff/evidence/20260926-heating-block-pilot-review');target.with_suffix('.json').write_text(json.dumps(result,indent=2)+'\n')
    ...
    ax.axhline(.5,ls='--',color='black');ax.set(xticks=x,xticklabels=['Block 24','Block 48'],ylabel='Actual local defect / original defect',title='Fixed-halo local improvement; GMRES not converged');ax.legend()
