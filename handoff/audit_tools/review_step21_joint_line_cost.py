...
def review(archive,receipt,received,terminal,output):
    ...
    assert term['job_id']==77905 and d['controls']['minimum_l2_gain']==.2
    ...
    prior=Path('outputs/review-20260925/joint-block-global-77843-received')
    audit=read(Path('handoff/evidence/20260926-joint-global-review.json'));claim=audit['archive']
    ...
    best=0. if dd==0 or rd>=0 or upper==0 else min(-rd/dd,upper,1.)
    if best:
        close(best,choice['best_fraction_without_safety'])
        close(math.sqrt((s0+2*best*rd+best*best*dd)/s0),choice['best_feasible_l2_ratio'])
    close(t,choice['fraction']);ratio=math.sqrt((s0+2*t*rd+t*t*dd)/s0) if t else 1.
    close(ratio,choice['predicted_l2_ratio']);checks={'nonzero_step':t>0,'l2_cost_pass':t>0 and ratio<=.8}
    assert choice['l2_cost_pass']==checks['l2_cost_pass']
    f=np.asarray(p['fluxes']);assert f.shape==(4,9632) and np.isfinite(f).all()
    if t:
        assert p['passes']==3
        checks.update(streamed_l2_cost_pass=choice['streamed_l2_ratio']<=.8,streamed_linf_nonincrease=choice['streamed_linf_ratio']<=1.0000000001,
