...
SOURCE=ROOT/'outputs/review-20260925/step21-wide-validation-77371-complete-received'
AUDIT=ROOT/'handoff/evidence/20260925-wide-validation-complete-review.json'
...
def main():
    ...
    protocol=json.loads(checked('control/pair11/feedback_protocol.json').read_text())
    ...
    vectors={n:{e:arrays(f'control/pair{n:02d}/{e}_response.npz')['residual'].reshape(128,4)
                for e in ('previous','final')} for n in (3,11)}
    component={}
    for a in ('previous','final'):
        for b in ('previous','final'):
            d=vectors[11][a]-vectors[3][b];sq=weights[:,None]*d*d
            fraction=sq.sum(0)/sq.sum()
            key=a+'_vs_'+b
            got=np.sqrt(sq.sum())/r20_mass_norm
            expected=audit['windows']['11']['window']['vector_difference_over_frozen_r20_norms'][key][1]
    ...
    # The largest audited window combination; nonlinear log allocation uses its exact secant factor.
    before=arrays('control/pair03/previous_energy_ledger.npz')
    after=arrays('control/pair11/final_energy_ledger.npz')
    ...
    np.testing.assert_allclose(logchange,(vectors[11]['final']-vectors[3]['previous'])[:,0],rtol=1e-8,atol=3e-14)
    parts=[]
    for i in range(76):
        def heating(n,e):
        ...
        dq=heating(11,'final')-heating(3,'previous')
    ...
    # Exploratory scalar heating projection on two REAL map pairs x1->x2, x9->x10.
    # This is a linear sensitivity proxy, not a new transfer solution or material gate.
    early_final=arrays('control/pair03/final_energy_ledger.npz')
    late_previous=arrays('control/pair11/previous_energy_ledger.npz')
    ...
    projected={'alpha':alpha,'paired_inputs':['x1','x9'],'paired_outputs':['x2','x10'],
    ...
    report={'source_archive':claim,'input_files_checked':len(used),'used_input_claims':used,
        ...
        'component_comparisons':component,'decomposition_pair':'pair11 final minus pair03 previous',
    ...
    out=ROOT/'handoff/evidence/20260926-feedback-drift-localization'
